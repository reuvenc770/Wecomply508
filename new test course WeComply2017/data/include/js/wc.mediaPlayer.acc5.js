...
wc.mediaPlayer.cleanup = function () {
	try {
		...
		try {
		if (isDefined(wc.mediaPlayer.video.activeElement.controls) && wc.mediaPlayer.video.activeElement.controls.isAvailable('Stop'))
				wc.mediaPlayer.video.activeElement.controls.stop();
		} catch (videocleanup) {
		alert('error in video cleanup ' + videocleanup);
		}
...
//alert('cleaned up');
		...
		alert('error thrown in media cleanup ' + e);
	}
};
...
wc.mediaPlayer.video.stop = function() {
	if (wc.mediaPlayer.status.platform == wc.mediaPlayer.platforms.html5 && (typeof (wc.mediaPlayer.video.activeElement) != 'undefined') && (wc.mediaPlayer.video.activeElement != null)) {
		...
		if (wc.mediaPlayer.video.activeElement != null) {
		if (isDefined(wc.mediaPlayer.video.activeElement.controls)) {
		if (wc.mediaPlayer.video.activeElement.controls != null) {
			try {
...
try {
					if (wc.mediaPlayer.video.activeElement.controls != 'undefined' && wc.mediaPlayer.video.activeElement.controls.isAvailable('Stop'))
						wc.mediaPlayer.video.activeElement.controls.stop();
} catch (videostopexception) {
// ie 8 on windows 7 throws this error, but video stops anyway, so ignore it
//alert('error thrown in video stop');
}
...
//alert('about to add next video');
			...
			//		containerElm.appendChild(embedelement);
				...
				wc.mediaPlayer.video.activeElement.URL = wc.mediaPlayer.flashvideodatafilepath + param.fileSrc;
					...
					if(isDefined(wc.mediaPlayer.video.activeElement.controls) ) {
					//if (wc.mediaPlayer.video.activeElement.controls != 'undefined') {
					if (wc.mediaPlayer.video.activeElement.controls != null) {
					if (wc.mediaPlayer.video.activeElement.controls.isAvailable('play')) {
...
wc.mediaPlayer.controls.autoUpdate = function() {
	try {
		...
		alert('error thrown in audoUpdate ' + e);
	}
};
			...
			if (wc.mediaPlayer.support.flashAudio()) {
				...
				if (typeof (wc.mediaPlayer.audio.activeElement) == 'undefined' || wc.mediaPlayer.audio.activeElement == null) {
					...
					try {
						...
						alert('error thrown in audio removechild ' + e);
					}
				}
