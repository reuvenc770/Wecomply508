...
wc.interface.actionHandler = {
	...
	quizQuestion_MultipleChoiceOneAnswerSelected: function (choiceIndex, elementType) {
		...
		if (isCorrect == false) {
			...
			if (isDefined(wc.data.values.elementsName) && wc.interface.currentElement.elementType == wc.interface.elementTypes.popQuestion && (wc.data.values.elementsName == 'iPadFrame' || wc.data.values.elementsName == 'WeComply2013' || wc.data.values.elementsName == 'iPadFrameAccenture2' || wc.data.values.elementsName == 'iPadFrameAccenture4')) {
				if (isDefined(tryAgainElement)) {
