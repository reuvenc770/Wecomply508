		...
		if (nextChapter.chapterOptions.variety == 'Memorandum') {
			...
			if (templateHtml.indexOf("Content:MuteButton") > 0) {
				if (wc.interface.options.muteAll)
					templateHtml = templateHtml.replaceTag('Content', 'MuteButton', "UnMute Audio/Video");
				else
					templateHtml = templateHtml.replaceTag('Content', 'MuteButton', "Mute Audio/Video");
			}
...
wc.interface.displaySecondLayer = function (elementType, onDoneCallback) {
	...
	if (elementType == wc.interface.elementTypes.help) {
		...
		for (var i = 0; i < wc.data.jsonData.chapters.length; i++) {
			...
			if (i <= wc.interface.currentElement.chapterIndex || (wc.interface.isAdmin && i<=5)) {
...
wc.interface.actionHandler = {
	...
	toggleMuteAll: function () {
		if (wc.interface.options.muteAll) {
			...
			//change the link
			document.getElementById("muteButton").innerHTML = "Mute Audio/Video";
		}
		else {
			...
			//change the link
			document.getElementById("muteButton").innerHTML = "UnMute Audio/Video";
		}
	},
...
wc.interface.transition = {
	...
	showModalWin_all: function () {
		...
		if (wc.interface.options.pageChange.popupReplacementMethod == wc.interface.transitionOptions.slideDownUp) {
			$("<div/>").addClass("modal-overlay-aux").width($(document).width()).height($(document).height()).appendTo("body").fadeTo("650", 0.7).zIndex(-1);
	...
	hideModalWin_all: function (resumeAudio) {
