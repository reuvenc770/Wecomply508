    ...
    isAdmin: false,
        ...
        linkBulletinTypes: {
            Code: {
                ...
                varietyCode: "Code",
                varietyName: "Code",
                resourceName: "CodeExcerpt"
            },
            News: {
                id: 2,
                varietyCode: "News",
                varietyName: "News",
                resourceName: "InTheNews"
            },
            FAQs: {
                id: 3,
                varietyCode: "FAQs",
            ...
            DosAndDonts: {
                id: 5,
                varietyCode: "DosAndDonts",
                varietyName: "Do's and Dont's",
                resourceName: "DosAndDonts"
            },
            CodeCompanion: {
                id: 6,
                varietyCode: "CodeCompanion",
                varietyName: "Code Companion",
                resourceName: "CodeCompanion"
            }
        },
...
wc.init = function(data, dataFilePath) {
    if (!wc.interface.isAdmin) {
        if (isDefined(dataFilePath)) wc.interface.dataFilePath = dataFilePath;
        ...
        //TODO: show error to user or at least log it
        ...
        if (
            wc.data.jsonData.language == "1" ||
            wc.data.jsonData.language == "13" ||
            wc.data.jsonData.language == "32"
        ) {
        ...
        if (!wc.WindowLoaded ||
            typeof wc.lms.window != "object" ||
            wc.lms.window.bReady != true
        ) {
            ...
            if (wc.interface.trycount > 3) {
                ...
                if (
                    ...
                    } else wc.interface.elements.intro.display();
                }
            }
        } else {
            if (wc.lms.delayStart) {
                ...
                if (
                    wc.interface.allowFlashInterface &&
                    wc.data.values["isFlashCourse"] == "1"
                ) {
                    //first check if flash is available and a good version before deciding to actually use the flash course
                    if (
                        isDefined(wc.flashInterface) &&
                        wc.flashInterface.isSupportedFlashVersion()
                    )
                        wc.interface.type = wc.interface.types.flash;
                    else wc.interface.type = wc.interface.types.html;
                } else wc.interface.type = wc.interface.types.html;
                ...
                if (wc.lms.objInfo != null) {
                    ...
                    if (typeof wc.lms.objInfo.bookmark != "undefined") {
                        if (
                            wc.lms.objInfo.bookmark == "-1" ||
                            wc.lms.objInfo.bookmark == "-2" ||
                            wc.lms.objInfo.bookmark == "GC"
                        ) {
                            if (wc.lms.objInfo.bookmark == "GC") wc.lms.bookmark = bm = "-1";
                        ...
                        if (
                            ...
                            return wc.interface.displayLayer(
                                wc.interface.elementTypes.bookmark,
                                bm
                            );
                        }
                    }
...
wc.interface.loadGameTemplates = function(variety) {
    if (wc.interface.gameFilePath == "/Wc2/training/GameData.aspx") {
        var path = "";
        path =
            "/Wc2/training/GameData.aspx?templatePlatform=" +
            wc.data.values.templatePlatform +
            "&styleName=" +
            wc.data.values.elementsName +
            "&gameVariety=" +
            variety;
        //console.log(path);
        wc.ajax.get(
        ...
        wc.ajax.get(
            variety + "." + wc.data.values.templatePlatform + ".Game.html",
            function(data) {
                for (var itemKey in data.responseJson) {
                    //if (typeof (wc.data.templateElements[itemKey]) == 'undefined' || wc.data.templateElements[itemKey] == null)
                    wc.data.templateElements[itemKey] = data.responseJson[itemKey];
                    wc.data.templateElements[itemKey] = wc.data.templateElements[
                        itemKey
                    ].replace(
                        /\/wc2\/static\/training\/ceoImages/g,
                        "images/iPhone/iPhone/ceoImages"
                    );
                    wc.data.templateElements[itemKey] = wc.data.templateElements[
                        itemKey
                    ].replace(
                        /\/wc2\/static\/training\/rubeGoldberg/g,
                        "images/iPhone/iPhone/rubeGoldberg"
                    );
                }
            },
            null,
...
wc.interface.replaceCurrentElement = function(
    nextElementType,
    nextChapterIndex,
    onDoneCallback
) {
    ...
    if (
        ...
        $("#SecondLayerElementContainer").removeClass(
            "LayerElementContainerCustomized"
        );
    }
    ...
    if (
        isDefined(wc.data.jsonData.description) &&
        wc.data.jsonData.description.length > 0
    ) {
    ...
    //TODO: add animation
    var currentElementContainer = document.getElementById(
    ...
    if (nextElementType == wc.interface.elementTypes.chapter) {
        ...
        if (typeof nextChapter.heading == "undefined") nextChapter.heading = "";
  ...
  $(document).attr("title", nextChapter.heading);
            ...
            else templateHtml = wc.data.templateElements.wideChapter_withVideo;
  ...
  $(document).attr("title", nextChapter.heading);
            ...
            if (
                wc.interface.doBullets &&
                isDefined(nextChapter.audio) &&
                isDefined(nextChapter.pithyQuote) &&
                wc.interface.elements.mediaBullets.hasBullets(nextChapter.pithyQuote) &&
                !wc.interface.options.muteAll
            ) {
                chapterMediaBulletsListHtml = wc.interface.elements.mediaBullets.render(
                    nextChapter.pithyQuote
                );
            }
            ...
            if (
                isDefined(nextChapter.video) &&
                isDefined(nextChapter.video.transcript) &&
                nextChapter.video.transcript.length != 0
            ) {
                ...
                templateHtml = templateHtml.replaceTag(
                    ...
                    "Chapter_Discussion",
                    "<h1>Transcript</h1>" +
                    nextChapter.video.transcript
                    .replace(/font-size:8pt/g, "font-size:1.24em")
                    .replace(/font-size: 8pt/g, "font-size:1.24em")
                );
            } else {
                templateHtml = templateHtml.replaceTag(
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Chapter_MediaBulletsList",
                chapterMediaBulletsListHtml
            );

            //LinkBulletin_Buttons/Menu
            var linkBulletinButtons = "";
            var linkBulletinMenuItems = "";
            for (var linkBulletinIndex in wc.data.jsonData.chapters[nextChapterIndex]
                    .linkBulletins) {
                if (
                    wc.interface.options.linkBulletin.displayMode ==
                    wc.interface.linkBulletinDisplayModes.menu
                ) {
                    var linkBulletinMenuItemTemplate =
                        wc.data.templateElements.linkBulletin_MenuItem;
                    linkBulletinMenuItems += linkBulletinMenuItemTemplate
                        .replaceTag(
                            "Content",
                            "LinkBulletin_Variety",
                            wc.data.jsonData.chapters[nextChapterIndex].linkBulletins[
                                linkBulletinIndex
                            ].variety
                        )
                        .replaceTag(
                    ...
                    wc.interface.options.linkBulletin.displayMode ==
                    wc.interface.linkBulletinDisplayModes.icons
                ) {
                    //default option
                    var linkBulletinButtonTemplate =
                        wc.data.templateElements.linkBulletin_Button;
                    linkBulletinButtons += linkBulletinButtonTemplate
                        .replaceTag(
                            "Content",
                            "LinkBulletin_Variety",
                            wc.data.jsonData.chapters[nextChapterIndex].linkBulletins[
                        ...
                        .replaceTag("Content", "LinkBulletin_VarietyResource", function() {
                            switch (wc.data.jsonData.chapters[nextChapterIndex].linkBulletins[
                                linkBulletinIndex
                            ].variety.toLowerCase()) {
                                case "news":
                            ...
                            return wc
                                .data.jsonData.chapters[nextChapterIndex].linkBulletins[linkBulletinIndex].variety;
                        });
                }
            }
            templateHtml = templateHtml.replaceTag(
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "LinkBulletin_MenuItems",
                linkBulletinMenuItems
            );
            ...
            if (!wc.interface.options.muteAll) {
                wc.interface.elements.mediaBullets.currentDisplayMode =
            ...
            if (
                ...
                if (
                    ...
                    wc.interface.mediaBulletinsDisplayTypes.bullets ||
                    wc.interface.elements.mediaBullets.currentDisplayMode ==
                    wc.interface.mediaBulletinsDisplayTypes.none
                ) {
                    wc.interface.elements.mediaBullets.currentDisplayMode =
                        wc.interface.mediaBulletinsDisplayTypes.bullets;
                    buttonsToActivate |= wc.interface.navigation.buttonTypes.Bullets;
        ...
        if (wc.interface.options.is508) {
            terms508linkaddition =
                '<span class="offscreen">Link to more information and additional resources  </span>';
        }
        var termslinkreplacement =
            '<a href="javascript:;" tabindex="0" id="Term_$1" class="wc-term-link" onclick="wc.interface.actionHandler.displayTerm($1);">' +
            terms508linkaddition;

        templateHtml = templateHtml.replace(
            /<a href="term:([0-9]+)">/g,
            termslinkreplacement
        );
        templateHtml = wc.interface.customNavigation.replaceTagsWithJavaScript(
            templateHtml
        );
        ...
        if (
            ...
            if (
                isDefined($(".chapterLinkBulletinLayout")) &&
                !$(".chapterLinkBulletinLayout").hasClass("hiddenElement")
            )
                $(".chapterLinkBulletinLayout").addClass("hiddenElement");
            if (
                isDefined($(".chapterLinkBulletinMenuLayout")) &&
                $(".chapterLinkBulletinMenuLayout").hasClass("hiddenElement") &&
                linkBulletinMenuItems != ""
            )
        ...
        wc.interface.handlers.fireEvent(
            wc.interface.handlers.onBefore_ReplaceCurrentElement, {
                elementType: nextElementType,
                nextChapter: wc.data.jsonData.chapters[nextChapterIndex]
            }
        );
        ...
        if (isDefined(document.getElementById("Button_Next"))) {
            if (isDefined(nextChapter.survey)) {
                ...
                if (
                    isDefined(nextChapter.audio) &&
                    isDefined(wc.data.jsonData.waitForAudio) &&
                    wc.data.jsonData.waitForAudio == "1"
                ) {
                    //wc.interface.elements.nextButton.disable(true);
                    buttonsToHide |= wc.interface.navigation.buttonTypes.NextChapter;
                    buttonsToDisable |= wc.interface.navigation.buttonTypes.NextChapter;
        ...
        // Media Bulletins
        var discussionContent = document.getElementById(
            "Chapter_Discussion_Content"
        );
        var mediaBulletins = document.getElementById(
            "Chapter_Discussion_MediaBulletins"
        );
        if (isDefined(mediaBulletins) && isDefined(discussionContent)) {
            if (
                wc.interface.doBullets &&
                wc.interface.showBullets &&
                wc.interface.elements.mediaBullets.hasBullets(nextChapter.pithyQuote) &&
                wc.interface.elements.mediaBullets.currentDisplayMode ==
                wc.interface.mediaBulletinsDisplayTypes.bullets
            ) {
        ...
        if (cpic) {
            cpic.innerHTML = document
                .getElementById("Chapter_PageInfo_Content")
                .innerHTML.replace("$1", nextChapterIndex + 1)
                .replace("$2", wc.data.jsonData.chapters.length);
            var pagesProgressBar = document.getElementById("pagesProgressBar");
            if (isDefined(pagesProgressBar)) {
                var percent = Math.round(
                    (nextChapterIndex + 1) * 100 / wc.data.jsonData.chapters.length
                );
        ...
        if (nextChapter.discussion == "" && isDefined(nextChapter.popQuestion)) {
            if (typeof nextChapter.popQuestion != "undefined") {
                ...
                wc.interface.currentElement.displayType =
                    wc.interface.displayTypes.Container;
                wc.lms.updateBookmark(nextChapterIndex + 1);
                return wc.interface.actionHandler.displayQuizQuestion(
                    nextChapter.popQuestion,
        ...
        var illustrationElement = document.getElementById(
            "Chapter_Illustration_Content"
        );
        if (typeof nextChapter.illustration != "undefined") {
            if (isDefined(illustrationElement)) {
                wc.interface.elements.illustration.inject(
                    "Chapter_Illustration_Content",
                    nextChapter.illustration,
                    true
                );
            } else {
                illustrationElement = $(".backgroundIllustrationContainer");
                if (illustrationElement.length > 0) {
                    illustrationElement.css(
                        "background-image",
                        'url("' +
                        wc.interface.dataFilePath +
                        nextChapter.illustration.src +
                        '")'
                    );
                }
            }
            ...
            if (isDefined(illustrationElement)) {
                ...
                document.getElementById("Chapter_Illustration_Content").style.display =
                    "none";
            }
        }
        ...
        if (
            nextChapter.chapterOptions.variety == "Memorandum" &&
            typeof nextChapter.video == "undefined"
        ) {
        ...
        if (
            ...
            var currentElementopacityTween = new OpacityTween(
                document.getElementById("LayerElementContainer"),
            ...
            currentElementopacityTween.onMotionFinished = function() {
                ...
                var layerElementopacityTween = new OpacityTween(
                    document.getElementById("CurrentElementContainer"),
                    Tween.regularEaseOut,
                    0,
                    100,
                    0.2
            ...
            var t1 = new Tween(
                document.getElementById("CurrentElementContainer").style,
                "left",
                Tween.regularEaseOut,
                0 - wc.GetWindowWidth(),
                0,
        ...
        if (
            wc.interface.options.pageChange.replacementMethod ==
            wc.interface.animationOptions.slideLeft ||
            wc.interface.options.pageChange.replacementMethod ==
            wc.interface.animationOptions.slideLeftAndFade
        ) {
            if (
                wc.interface.options.pageChange.replacementMethod ==
                wc.interface.animationOptions.slideLeftAndFade
            ) {
                currentElementopacityTween = new OpacityTween(
                    document.getElementById("CurrentElementContainer"),
                    Tween.regularEaseOut,
                    0,
                    100,
            ...
            //if (wc.interface.options.pageChange == wc.interface.animationOptions.slideLeft) {
            //TODO: implement better way to put opacity back
            currentElementopacityTween = new OpacityTween(
                document.getElementById("CurrentElementContainer"),
                Tween.regularEaseOut,
        ...
        if (
            typeof wc.data.jsonData.chapters[nextChapterIndex].video != "undefined" &&
            wc.data.jsonData.chapters[nextChapterIndex].video != null
        ) {
            //we dont want to play any audio/video when we come from Admin
            if (!wc.interface.isAdmin &&
                wc.data.jsonData.chapters[nextChapterIndex].chapterOptions.autoplay ==
                "true"
            ) {
                if (!wc.interface.options.muteAll)
        ...
        if (
            typeof wc.data.jsonData.chapters[nextChapterIndex].audio != "undefined" &&
            wc.data.jsonData.chapters[nextChapterIndex].audio != null
        ) {
            //console.log('heyyyyyy, audio started!');
            //we dont want to play any audio/video when we come from Admin
            if (!wc.interface.isAdmin &&
                wc.interface.options.mediaPlayer.allowAudioAutoPlay &&
                wc.data.jsonData.chapters[nextChapterIndex].chapterOptions.autoplay ==
                "true" &&
                !wc.interface.options.muteAll
            ) {
                ...
                wc.mediaPlayer.logs.push(
                    "Calling audio.play from replaceCurrentElement"
                );
        ...
        if (wc.interface.options.is508) {
            ...
            setTimeout(function() {
                var element = $("#CurrentElementContainer")
                    .find("*[tabindex]")
                    .filter(":visible")
                    .filter(":first");
    ...
    if (wc.interface.showBullets && chapterMediaBulletsListHtml != "") {
        ...
        if (
            isDefined(Button_Bullets) &&
            wc.interface.elements.general.getClassName(Button_Bullets) !=
            "bulletsButton_Disabled"
        ) {
            ...
            buttonsToEnable |=
                ...
                wc.interface.navigation.buttonTypes.Text ?
                wc.interface.navigation.buttonTypes.None :
                wc.interface.navigation.buttonTypes.Text;
        }
    } else {
        var Button_Bullets = document.getElementById("Button_EnableBullets");
    ...
    if (
        wc.data.jsonData.chapters[nextChapterIndex].chapterOptions.pagination ==
        "false"
    ) {
        try {
    ...
    if (
        nextChapterIndex != 0 &&
        wc.data.jsonData.waitForAudio == "0" &&
        nextChapterIndex + 2 > wc.lms.highestBookmark
    ) {
...
wc.interface.buildSurveyQuestionHtml = function(survey, choiceIndex) {
    ...
    for (var questionIndex in survey.surveyQuestions) {
        ...
        if (question.variety == "MultipleChoiceMultipleAnswers") {
            choiceTemplateHtml =
                wc.data.templateElements.survey_MultipleChoiceMultipleAnswer;
            ...
            for (var choiceIndex in question.choices) {
                ...
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Survey_MultipleChoiceMultipleAnswer_Body",
                    question.choices[choiceIndex].body
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Survey_MultipleChoiceMultipleAnswer_Name",
                    "Survey_Choices"
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Survey_MultipleChoiceMultipleAnswer_Id",
                    choiceIndex
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
            ...
            questionTemplateHtml = questionTemplateHtml.replace(
                /width:\d+px;/gi,
                "width:" + question.width + ";"
            );
            questionTemplateHtml = questionTemplateHtml.replaceTag(
                "Content",
                "idSuffix",
                "_subQuestion" + questionIndex
            );
            questionTemplateHtml = questionTemplateHtml.replaceTag(
                "Content",
                "Survey_Body",
            ...
            choiceHtml = choiceHtml.replaceTag(
                "Content",
                "elementType",
                wc.interface.elementTypes.survey
            );
            ...
            choiceTemplateHtml =
                wc.data.templateElements.survey_MultipleChoiceOneAnswer;
            questionTemplateHtml = wc.data.templateElements.survey;
            ...
            for (var choiceIndex in question.choices) {
                ...
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Survey_MultipleChoiceOneAnswer_Body",
                    question.choices[choiceIndex].body
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Survey_MultipleChoiceOneAnswer_Name",
                    "Survey_Choices"
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Survey_MultipleChoiceOneAnswer_Id",
                    choiceIndex
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
            ...
            choiceTemplateHtml = choiceTemplateHtml.replaceTag(
                "Content",
                "Survey_TextEssay_Id",
                choiceIndex
            );
            ...
            choiceTemplateHtml = choiceTemplateHtml.replaceTag(
                "Content",
                "Survey_TextEssay_height",
                question.height
            );
            choiceTemplateHtml = choiceTemplateHtml.replaceTag(
                "Content",
                "Survey_TextEssay_width",
                question.width
            );
            choiceTemplateHtml = choiceTemplateHtml.replaceTag(
                "Content",
                "Survey_TextEssay_Label",
                question.body
            );
            choiceTemplateHtml = choiceTemplateHtml.replaceTag(
                "Content",
...
wc.interface.buildSurveyHtml = function(survey, choiceIndex) {
    ...
    for (var choiceIndex in survey.pivotQuestion.choices) {
        ...
        tmpHtml = tmpHtml.replaceTag(
            "Content",
            "Survey_PivotQuestion_Body",
            survey.pivotQuestion.choices[choiceIndex].body
        );
        tmpHtml = tmpHtml.replaceTag(
            "Content",
            "Survey_PivotQuestion_Id",
            choiceIndex
        );
        tmpHtml = tmpHtml.replaceTag(
            "Content",
...
wc.interface.game = {
    ...
    init: function(gameIndex) {
        ...
        wc.interface.game.variety = wc.data.jsonData.quiz.games[
            gameIndex
        ].variety.replace("Both", "");
        ...
        wc.interface.game.questionChapters = wc.tools.shuffle(
            wc.interface.game.questionChapters
        );
    },
        ...
        if (choice.isCorrect == "false") {
            ...
            if (
                ...
                if (
                    ...
                    wc.interface.game.currentQuestions.length == 1
                ) {
                    if (wc.interface.game.correctAnswerOnFirstTry == null) {
                    ...
                    if (
                        wc.interface.game.questionsAskedPoolCount <
                        wc.interface.game.questionChapters.length
                    ) {
                        ...
                        if (
                            ...
                            wc.interface.game.currentQuestions =
                            wc.data.jsonData.quizQuestions;
                        else
                            wc.interface.game.currentQuestions =
                        ...
                        wc.interface.game.currentQuestions = wc.tools.shuffle(
                            wc.interface.game.currentQuestions
                        );
                    } else {
                        wc.interface.game.currentQuestions = null;
    ...
    loadQuestion: function() {
        if (wc.interface.game.gameIndex == null) return null;
    ...
    reportScore: function(correctQuestions, totalQuestions, bGame) {
        // bGame = true if program has a game
        if (true == wc.lms.window.bNoLMS) return true;
...
wc.interface.displayLayer = function(elementType, data, onDoneCallback) {
    ...
    wc.interface.handlers.fireEvent(wc.interface.handlers.onBefore_DisplayLayer, {
    ...
    if (elementType == wc.interface.elementTypes.game) {
        ...
        if (
            ...
            (typeof wc.data.jsonData.finalQuizQuestion == "undefined" ||
                wc.data.jsonData.finalQuizQuestion == "")
        ) {
            wc.interface.navigateToElement(wc.interface.directions.next);
        } else {
            var choiceHtml = "";
            for (var choiceIndex in !wc.interface.game.chapterQuestionsCompleted &&
                    wc.interface.options.games.randomizeChoices ?
                    wc.tools.shuffle(question.choices) :
                    question.choices) {
                var tmpHtml = itemHtml;
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "Quiz_ChoiceItem_Body",
                    question.choices[choiceIndex].body
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "ContinueResource",
                wc.interface.getResource("continue")
            );

            var layerElementContainer = document.getElementById("Quiz_GameContent");
            var layerElementContainerForFading = document.getElementById(
                "Quiz_GameContent"
            ); //document.getElementById('LayerElementContainer')
            ...
            wc.interface.handlers.games.fireEvent(
                wc.interface.handlers.games.onBefore_LoadQuestion, { elementType: wc.interface.currentElement.elementType, data: null }
            );
            if (
                wc.interface.handlers.games.hasEvent(
                    wc.interface.handlers.games.insteadOf_LoadQuestion
                )
            ) {
                wc.interface.handlers.games.fireEvent(
                    wc.interface.handlers.games.insteadOf_LoadQuestion, {
                ...
                if (
                    wc.interface.currentElement.elementType ==
                    wc.interface.elementTypes.game &&
                    (navigator.userAgent.toLowerCase().indexOf("msie 7.0") >= 0 ||
                        navigator.userAgent.toLowerCase().indexOf("msie 8.0") >= 0)
                ) {
                    layerElementContainer.innerHTML = templateHtml;

                    if (typeof question.illustration != "undefined")
                        wc.interface.elements.illustration.inject(
                            "Quiz_Illustration_Content",
                    ...
                    var currentElementopacityTween = new OpacityTween(
                        wc.interface.currentElement.elementType ==
                        wc.interface.elementTypes.game ?
                        layerElementContainerForFading :
                        document.getElementById("CurrentElementContainer"),
                        Tween.regularEaseOut,
                        100,
                        0,
        ...
        wc.interface.handlers.games.fireEvent(
            wc.interface.handlers.games.onAfter_LoadQuestion, { elementType: wc.interface.currentElement.elementType, data: null }
        );
        ...
        var layerElementContainer = document.getElementById(
            "LayerElementContainer"
        );
        ...
        if (isDefined(templateHtml) && templateHtml != "") {
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Game_Intro_LoadingText",
                wc.interface.getResource("LoadingGame")
            );
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Game_Intro_WelcomeText",
                game.introduction || ""
            );
            templateHtml = templateHtml.replaceTag(
                "Content",
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Game_Intro_StartQuizText",
                wc.interface.getResource("StartQuiz")
            );
            templateHtml = templateHtml.replaceTag(
                "Content",
                "gameIndex",
        ...
        templateHtml = wc.data.templateElements.game_quiz_Wrapper.replaceTag(
            "Content",
            "quizContent",
            templateHtml
        );
        templateHtml = wc.data.templateElements.layerWrapper.replaceTag(
            "Content",
            "LayerContent",
            templateHtml
        );
        ...
        if (skipIntro) {
            ...
            wc.interface.currentElement.elementType = wc.interface.elementTypes.game;
            return wc.interface.actionHandler.displayGame(
                wc.interface.game.gameIndex
            );
        }
        ...
        if (
            wc.interface.options.games.gameIntroDisplayAnimation ==
            wc.interface.animationOptions.fadeOutIn
        ) {
            var currentElementopacityTween = new OpacityTween(
                wc.interface.currentElement.displayType == 1 ?
                document.getElementById("CurrentElementContainer") :
                document.getElementById("LayerElementContainer"),
                Tween.regularEaseOut,
                100,
            ...
            currentElementopacityTween.onMotionFinished = function() {
                ...
                wc.interface.handlers.games.fireEvent(
                    wc.interface.handlers.games.onAfter_DisplayIntroLayer, { elementType: wc.interface.currentElement.elementType, data: null }
                );
            };
        ...
        for (var gameIndex in wc.data.jsonData.quiz.games) {
            ...
            var gameItemHtml = itemHtml.replaceTag(
                "Content",
                "GameItem_Title",
                game.name
            );
            gameItemHtml = gameItemHtml.replaceTag(
                "Content",
                "GameItem_Body",
                game.description
            );
        ...
        if (templateHtml.indexOf("gameChoiceBoxBackground") >= 0) {
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Game_Introduction",
                wc.data.jsonData.quiz.introduction
            );
        }
        ...
        if (
            wc.interface.options.games.gameChoiceDisplayAnimation ==
            wc.interface.animationOptions.fadeOutIn
        ) {
            var currentElementopacityTween = new OpacityTween(
                wc.interface.currentElement.displayType == 1 ?
        ...
        setTimeout(function() {
            $("#LayerElementContainer").css("display", "");
        }, 1000);
        //wc.interface.elements.gamechoice.resizeFont(true);
        wc.interface.currentElement.elementType =
            wc.interface.elementTypes.gameChoice;
        ...
        if (isDefined(wc.data.jsonData.certificate)) {
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Certificate_Footer",
                wc.data.jsonData.certificate.footer
            );
            templateHtml = templateHtml.replaceTag(
                "Values",
                "FirstName",
            ...
            if (typeof wc.data.jsonData.certificate.illustration != "undefined")
                wc.interface.elements.illustration.inject(
                    "Certificate_UserDefined_Illustration_Content",
                    wc.data.jsonData.certificate.illustration,
                    true
                );
            else {
                //no illustration found
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Certificate_ProgramName",
                wc.data.jsonData.name
            );
            templateHtml = templateHtml.replaceTag(
                "Content",
                "SurveyUrl",
                wc.interface.elements.survey.surveyUrl()
            );
            //templateHtml = templateHtml.replaceTag('Content', 'Acknowledgment_Body', wc.data.jsonData.acknowledgment.body);
            ...
            templateHtml = templateHtml
                .replace("{$Value:CompanyAddress}", "")
                .replace("{$Value:CompanyCity}", "");
            layerElementContainer.innerHTML = templateHtml;

            if (wc.interface.elements.survey.hasAnsweredQuestions)
                document.getElementById("SurveyAnswers").style.display = "";
            else document.getElementById("SurveyAnswers").style.display = "none";
        ...
        if (
            isDefined(SendCertificateContainer) &&
            wc.data.jsonData.sendCertificate == "1"
        ) {
            SendCertificateContainer.style.display = "";
            var clickFunction =
                "javascript:wc.interface.actionHandler.sendCertificate(" +
                wc.lms.objInfo.aicc_sid +
                ");";
            SendCertificateContainer.firstChild.setAttribute(
                "onclick",
                clickFunction
            );
        }
        ...
        document.getElementById("LayerElementContainer").style.display = "";
        wc.interface.currentElement.elementType =
            wc.interface.elementTypes.certificate;
        ...
        if (
            ...
            if (wc.lms.objInfo.bookmark == "-2") {
                templateHtml = templateHtml.replaceTag(
                    "Content",
                    "Bookmark_Question",
                    wc.interface.getResource("PodcastDownloaded")
                ); //usedInJS" select="true()
                templateHtml = templateHtml.replaceTag(
                    "Content",
                    "Bookmark_Answer1",
                    wc.interface.getResource("PodcastReviewed")
                );
                templateHtml = templateHtml.replaceTag(
                    "Content",
                    "Bookmark_Answer2",
                    wc.interface.getResource("PodcastRestartCourse")
                );
            } else {
                templateHtml = templateHtml.replaceTag(
                    "Content",
                    "Bookmark_Question",
                    wc.interface.getResource("PreviouslyCompleted")
                ); //usedInJS" select="true()
                templateHtml = templateHtml.replaceTag(
                    "Content",
                    "Bookmark_Answer1",
                    wc.interface.getResource("Proceedtogame")
                );
                templateHtml = templateHtml.replaceTag(
                    "Content",
                    "Bookmark_Answer2",
                    wc.interface.getResource("BeginWithLesson")
                );
            }
        } else {
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Bookmark_Question",
                wc.interface.getResource("PreviouslyBegun")
            ); //usedInJS" select="true()
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Bookmark_Answer1",
                wc.interface.getResource("Continuewith") + wc.lms.bookmark
            );
            templateHtml = templateHtml.replaceTag(
                "Content",
                "Bookmark_Answer2",
        ...
        if (document.all) {
            ...
            layerElementContainer.style.left = wc.GetObjectBoundaries(
                document.getElementById("CurrentElementContainer")
            ).right;
        }
        ...
        if (!$(layerElementContainer).hasClass("dontReposition")) {
            layerElementContainer.style.top = wc.GetObjectBoundaries(
                document.getElementById("CurrentElementContainer")
            ).top;
        }
        ...
        if ($continueButton.hasClass("disabled")) {
            wc.interface.actionHandler.helpers.disableDefaultFunctionality(
                $continueButton
            );
        }
        ...
        if (
            typeof document.getElementById("CurrentElementContainer") != "undefined"
        ) {
            currentElementopacityTween = new OpacityTween(
                document.getElementById("CurrentElementContainer"),
                Tween.regularEaseOut,
            ...
            currentElementopacityTween.onMotionFinished = function() {
                ...
                document.getElementById("LayerElementContainer").style.display = "";
                wc.interface.actionHandler.scrollToTop();
                layerElementopacityTween = new OpacityTween(
                    document.getElementById("LayerElementContainer"),
                    Tween.regularEaseOut,
                    0,
                    100,
        ...
        if (
            isDefined(wc.data.values.IsiPadFrame) &&
            wc.data.values.IsiPadFrame == "True"
        )
            wc.interface.actionHandler.pauseAudio(false);
        else wc.mediaPlayer.stopCurrentActiveMedia();
        ...
        templateHtml = templateHtml.replaceTag(
            ...
            wc.data.jsonData.acknowledgment.heading
        );
        templateHtml = templateHtml.replaceTag(
            "Content",
            "Acknowledgment_Body",
            wc.data.jsonData.acknowledgment.body
        );
        templateHtml = wc.interface.customNavigation.replaceTagsWithJavaScript(
            templateHtml
        );
        ...
        for (var problemIndex in data.problems) {
            ...
            if (wc.data.values.templatePlatform != "mobile" || i < 7) {
                ...
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "MatchGameItem_Body",
                    data.problems[problemBodies[problemIndex]].body
                );
                if (
                    wc.data.values.templatePlatform == "mobile" &&
                    data.problems[problemSections[problemIndex]].section.length > 23
                ) {
                    tmpHtml = tmpHtml.replaceTag(
                        "Content",
                        "MatchGameItem_Section",
                        data.problems[problemSections[problemIndex]].section.substr(0, 23)
                    );
                } else {
                    tmpHtml = tmpHtml.replaceTag(
                        "Content",
                        "MatchGameItem_Section",
                        data.problems[problemSections[problemIndex]].section
                    );
                }
                ...
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "bodyIndex",
                    problemBodies[problemIndex]
                );
                tmpHtml = tmpHtml.replaceTag(
                    "Content",
                    "sectionIndex",
                    problemSections[problemIndex]
                );
                problemHtml += tmpHtml;
            }
        }
        ...
        matchGameHtml = matchGameHtml.replaceTag(
            "Content",
            "MatchGame_GameContent",
            problemHtml
        );
        matchGameHtml = wc.interface.customNavigation.replaceTagsWithJavaScript(
            matchGameHtml
        );
        ...
        $('[data-droppable="true"]').droppable({
            ...
            drop: function(event, element) {
                ...
                var toGameItemElement = $(dropppable).closest(
                    '[id^="MatchGameItem_Element_"]'
                );
                var toQuestion = $('[id^="MatchGameItem_Body_"]', toGameItemElement);

                // get the old question (the one we dragged)
                var fromGameItemElement = element.draggable.closest(
                    '[id^="MatchGameItem_Element_"]'
                );
                var fromQuestion = $(
                    '[id^="MatchGameItem_Body_"]',
                    fromGameItemElement
                );
                ...
                for (var problemIndex in data.problems) {
                    ...
                    if (wc.data.values.templatePlatform != "mobile" || i < 7) {
                        ...
                        var questionId = $(
                                '[id^="MatchGameItem_Section_"]',
                                gameItemElement
                            )
                            .attr("id")
                            .substring("MatchGameItem_Section_".length);
                        var answerId = $('[id^="MatchGameItem_Body_"]', gameItemElement)
                            .attr("id")
                            .substring("MatchGameItem_Body_".length);
                        ...
                        if (questionId == answerId) {
                            ...
                            if (draggableElement.is(".ui-draggable")) {
                                ...
                                setTimeout(
                                    "$('[data-draggable=\"true\"]', $('#MatchGameItem_Element_" +
                                    problemIndex +
                                    "')).draggable('destroy')",
                                    200
                                );
                            ...
                            if (droppableElement.is(".ui-droppable")) {
                                ...
                                setTimeout(
                                    "$('[data-droppable=\"true\"]', $('#MatchGameItem_Element_" +
                                    problemIndex +
                                    "')).droppable('destroy')",
                                    200
                                );
                ...
                if (
                    ...
                    setTimeout(function() {
                        ...
                        var MatchGame_OutroContainer = document.getElementById(
                            "MatchGame_OutroContainer"
                        );
                        MatchGame_OutroContainer.style.display = "";

                        var MatchGame_Heading_Outro = document.getElementById(
                            "MatchGame_Heading_Outro"
                        );
                        if (isDefined(MatchGame_Heading_Outro)) {
                            matchGame_MainHeaderBar = document.getElementById(
                                "matchGame_MainHeaderBar"
                            );
        ...
        var choiceTemplateHtml = "";
        var continueButtonTemplate =
            wc.data.templateElements.quizQuestion_ContinueButton;
        ...
        if (data.variety == "MultipleChoiceOneAnswer")
            choiceTemplateHtml =
            wc.data.templateElements.quizQuestion_MultipleChoiceOneAnswer;
        ...
        for (var choiceIndex in data.choices) {
            ...
            tmpHtml = tmpHtml.replaceTag(
                "Content",
                "QuizQuestion_MultipleChoiceOneAnswer_Body",
                data.choices[choiceIndex].body
            );
            tmpHtml = tmpHtml.replaceTag(
                "Content",
                "QuizQuestion_MultipleChoiceOneAnswer_Name",
                "quizQuestion_Choices"
            );
            tmpHtml = tmpHtml.replaceTag(
                "Content",
                "QuizQuestion_MultipleChoiceOneAnswer_Id",
                choiceIndex
            );
        ...
        templateHtml = templateHtml.replaceTag(
            "Content",
            "QuizQuestion_Buttons",
            continueButtonTemplate
        );
        templateHtml = templateHtml.replaceTag(
            "Content",
            "QuizQuestion_Choices",
        ...
        if (typeof data.illustration != "undefined") {
            wc.interface.elements.illustration.inject(
                "QuizQuestion_Illustration_Content",
                data.illustration,
                true
            );
            var QuizQuestion_Feedback_Illustration_Content = document.getElementById(
                "QuizQuestion_Feedback_Illustration_Content"
            );
        ...
        if (
            ...
            wc.interface.actionHandler.pauseAudio(false);
        else wc.mediaPlayer.stopCurrentActiveMedia(true);
        ...
        if (typeof closeButton != "undefined" && closeButton != null) {
            ...
            closeButton.onKeyPress = function() {
                ...
                if (
                    wc.interface.handlers.fireEvent(
                        wc.interface.handlers.onBefore_CloseDisplayLayer, { elementType: wc.interface.currentElement.elementType, data: null }
                    )
                ) {
                    //transition hideModalWin
                    if (
                        ...
                        setTimeout(function() {
                            wc.interface.navigation.updateButtons(
                                buttonsCurrentState.buttonsToEnable,
                                buttonsCurrentState.buttonsToDisable,
                                buttonsCurrentState.buttonsToHide,
                                buttonsCurrentState.buttonsToActivate
                            );
                        }, 610);
                    } else {
                        currentElementopacityTween = new OpacityTween(
                ...
                wc.interface.navigation.insecondlayer = false;
                wc.interface.handlers.fireEvent(
                    wc.interface.handlers.onAfter_CloseDisplayLayer, { elementType: wc.interface.currentElement.elementType, data: null }
                );
            };
        }
        wc.interface.actionHandler.scrollToTop();
        ...
        elementType == wc.interface.elementTypes.linkBulletin ||
        elementType == wc.interface.elementTypes.popBulletin
    ) {
        //maybe we should save the state and use restorestate functionality
        wc.interface.navigation.saveButtonsCurrentState();
        if (
        ...
        linkBulletinTypeName =
            wc.data.resources[
                wc.interface.linkBulletinTypes[data.variety].resourceName.toLowerCase()
            ];
        ...
        templateHtml = templateHtml.replaceTag(
            "Content",
            "LinkBulletin_Subhead",
            data.subhead || ""
        );
        templateHtml = templateHtml.replaceTag(
            "Content",
            "LinkBulletin_Headline",
            data.headline || ""
        );
        ...
        if (wc.interface.linkBulletinTypes.News.varietyCode != data.variety) {
            if (templateHtml.indexOf("dosAndDontsHeader_Container") >= 0) {
                templateHtml = templateHtml.replace(
                    'class = "newsLayout"',
                    'class = "newsLayout hideDiv"'
                );
                templateHtml = templateHtml.replace(
                    'class="dosAndDontsHeader_Container hideDiv"',
                    'class="dosAndDontsHeader_Container"'
                );
            }
        }
        ...
        if (wc.interface.options.popup.customWidth) {
            if (!isDefined(data.illustration)) {
                $(".backgroundIllustrationContainer", layerElementContainer).addClass(
                    "hiddenElement"
                );
                $("#whiteboardContainer", layerElementContainer).addClass(
                    "singleColumn"
                );
                $("#whiteboardLinkBulletin", layerElementContainer).css(
                    "width",
                    data.width
                );
            }
        ...
        if (isDefined(data.video) && !wc.interface.options.muteAll) {
            ...
            wc.mediaPlayer.video.videoParams.containerElementId =
                "LinkBulletin_Video_Container";
            wc.mediaPlayer.video.play(wc.mediaPlayer.video.videoParams);
        }

        var illustrationElement = document.getElementById(
            "LinkBulletin_Illustration_Content"
        );
        if (typeof data.illustration != "undefined") {
            if (isDefined(illustrationElement)) {
                wc.interface.elements.illustration.inject(
                ...
                if (illustrationElement.length > 0) {
                    illustrationElement.css(
                        "background-image",
                        'url("' + wc.interface.dataFilePath + data.illustration.src + '")'
                    );
                }
            }
        } else {
        ...
        if (!$(layerElementContainer).hasClass("dontReposition")) {
            ...
            //layerElementContainer.style.top = wc.GetObjectBoundaries(document.getElementById('CurrentElementContainer')).top;
            layerElementContainer.style.top = document.getElementById(
                "CurrentElementContainer"
            ).style.top;
        }
        ...
        if (typeof closeButton != "undefined" && closeButton != null) {
            closeButton.onclick = function() {
                ...
                if (
                    ...
                    } else wc.interface.navigateToElement(wc.interface.directions.next);
                } else {
                    //transition hideModalWin
                    if (
                        wc.interface.options.pageChange.popupReplacementMethod ==
        ...
        if (
            ...
            currentElementopacityTween.onMotionFinished = function() {
                ...
                if (
                    wc.interface.options.displayLayer.linkBulletin.textDisplay ==
                    wc.interface.animationOptions.textWriter
                ) {
                    var linkBulletinBodyContent = document.getElementById(
                        "LinkBulletin_Body_Content"
                    );
                    ...
                    var bodyTextTween = new TextTween(
                        linkBulletinBodyContent,
                        "innerHTML",
                        bodyText,
                        Tween.strongEaseOut,
                        2
        ...
        if (typeof data.introduction == "undefined") data.introduction = ""; //Deal wuith it
        ...
        templateHtml = templateHtml.replaceTag(
            "Content",
            "quizHasVideo",
            isDefined(data.video) || ""
        );
        templateHtml = templateHtml.replaceTag(
            "Content",
            "PopQuiz_Illustration",
            data.illustration || ""
        );
        templateHtml = templateHtml.replaceTag(
            "Content",
        ...
        if (isDefined(data.video)) {
            ...
            templateHtml = templateHtml.replaceTag(
                "Content",
                "PopQuiz_Video_Transcript",
                data.video.transcript
                .replace(/font-size:8pt/g, "font-size:1.24em")
                .replace(/font-size: 8pt/g, "font-size: 1.24em") || ""
            );
            if (
                isDefined(data.video.alternateText) &&
                data.video.alternateText != ""
            ) {
            ...
            //No video and no introduction, go straight to the question
        ...
        templateHtml = templateHtml.replace(
            "{Content:MinimumPassingScore}",
            wc.data.jsonData.minimumPassingScore
        );
    ...
    wc.interface.handlers.fireEvent(wc.interface.handlers.onAfter_DisplayLayer, {
        elementType: elementType,
        data: data
    });
...
wc.interface.displaySecondLayer = function(
    elementType,
    onDoneCallback,
    options
) {
    wc.interface.handlers.fireEvent(
        wc.interface.handlers.onBefore_DisplaySecondLayer, { elementType: elementType }
    );
    ...
    var secondLayerElementContainer = document.getElementById(
    ...
    if (elementType == wc.interface.elementTypes.help) {
        ...
        for (var i = 0; i < wc.data.jsonData.chapters.length; i++) {
            ...
            tmpItemTemplate = tmpItemTemplate.replaceTag(
                "Content",
                "TableOfContents_Item_Text",
                wc.data.jsonData.chapters[i].heading
            );
            var clickFunction = "";
            var itemColor = "";
            ...
            if (
                ...
                i < wc.lms.highestPage ||
                i < wc.lms.highestBookmark ||
                opt.all
            ) {
                clickFunction =
        ...
        var notes = "";
        if (
            isDefined(wc.interface.storage.loadfromStorage(wc.lms.objInfo.aicc_sid))
        )
    ...
    if (!$(secondLayerElementContainer).hasClass("dontReposition")) {
        if (document.all) {
            ...
            secondLayerElementContainer.style.left = wc.GetObjectBoundaries(
                currentElementContainer
            ).right;
        }
        secondLayerElementContainer.style.top = wc.GetObjectBoundaries(
            currentElementContainer
        ).top;
    }
    wc.interface.actionHandler.scrollToTop();
    ...
    if (
        ...
        var elementToHide =
            ...
            currentElementContainer :
            layerElementContainer;
        var elementToHideOpacityTween = new OpacityTween(
            elementToHide,
            Tween.regularEaseOut,
            100,
            0,
        ...
        elementToHideOpacityTween.onMotionFinished = function() {
            ...
            var secondLayerElementOpacityTween = new OpacityTween(
                secondLayerElementContainer,
                Tween.regularEaseOut,
                0,
                100,
...
wc.interface.closeSecondLayer = function(elementType) {
    wc.interface.handlers.fireEvent(
        wc.interface.handlers.onBefore_CloseSecondLayer, { elementType: elementType }
    );
    ...
    if (
        ...
        setTimeout(function() {
            ...
            wc.interface.navigation.insecondlayer = false;
            wc.interface.handlers.fireEvent(
                wc.interface.handlers.onAfter_CloseSecondLayer, { elementType: elementType }
            );
        }, 610);
    } else {
        var elementToShow =
            ...
            currentElementContainer :
            layerElementContainer;
        var secondLayerOpacityTween = new OpacityTween(
            secondLayerElementContainer,
            Tween.regularEaseOut,
            100,
            0,
        ...
        secondLayerOpacityTween.onMotionFinished = function() {
            ...
            // TODO: reset font sizes
            var elementToShowOpacityTween = new OpacityTween(
                elementToShow,
                Tween.regularEaseOut,
                0,
                100,
...
wc.interface.navigateToElement = function(direction) {
    ...
    if (
        wc.interface.currentElement.elementType ==
        wc.interface.elementTypes.intro &&
        direction == wc.interface.directions.next
    ) {
        //Nothing to do at the moment
    } else if (
        wc.interface.currentElement.elementType ==
        wc.interface.elementTypes.intro &&
        direction == wc.interface.directions.previous
    ) {
        ...
        wc.interface.elementTypes.popBulletin ||
        wc.interface.currentElement.elementType ==
        wc.interface.elementTypes.popQuiz ||
        wc.interface.currentElement.elementType ==
        wc.interface.elementTypes.popQuestion ||
        wc.interface.currentElement.elementType ==
        ...
        wc.interface.currentElement.elementType == wc.interface.elementTypes.term ||
        wc.interface.currentElement.elementType ==
        wc.interface.elementTypes.matchGame ||
        wc.interface.currentElement.elementType == wc.interface.elementTypes.game
    ) {
        if (
            wc.interface.currentElement.chapterIndex >=
            wc.data.jsonData.chapters.length - 1
        ) {
                ...
                if (wc.interface.type == wc.interface.types.html) {
                   ...
                   // var matchGame =
                    ...
                    var elapsedTimeInSeconds = function() {
                        try {
                            return (
                                (wc.interface.getMyTime(new Date()) -
                                    wc.interface.beginTime.getTime()) /
                                1000
                            );
                        } catch (e) {
                            return 0;
                    ...
                    if (
                        (elapsedTimeInSeconds < minimumTimeInSeconds ||
                            minimumTimeInSeconds == 0) &&
                        typeof popQuestion != "undefined"
                    ) {
                        wc.interface.actionHandler.displayQuizQuestion(
                            popQuestion,
                            wc.interface.elementTypes.popQuestion
                        );
                        ...
                        wc.interface.actionHandler.displayMatchGame(
                            matchGame,
                            wc.interface.elementTypes.matchGame
                        );
    ...
    if (wc.lms.doBookmarkUpdate) {
        if (!wc.lms.updateBookmark(name)) return; // failed, don't continue with current navigation....
    } else {
...
wc.interface.navigateToElementAfterLastChapter = function() {
    ...
    if (
        ...
        if (typeof wc.data.jsonData.acknowledgment != "undefined")
            return wc.interface.actionHandler.displayAcknowledgment(
                wc.data.jsonData.acknowledgment
            );
        else if (
            typeof wc.data.jsonData.quiz != "undefined" &&
            typeof wc.data.jsonData.quiz.games != "undefined" &&
            wc.data.jsonData.quiz.games.length > 0
        )
...
wc.interface.elements = {
    intro: {
        display: function() {
            ...
            if (
                wc.data.templateElements.intro_Animation != "" &&
                wc.data.jsonData.description != "" &&
                ...
                for (i = 0; i < descriptions.length - 1; i++) {
                    var tmpItemTemplate =
                        wc.data.templateElements.intro_Animation_Element;
                    tmpItemTemplate = tmpItemTemplate.replaceTag(
                        "Content",
                        "Chapter_Description",
                        descriptions[i]
                    );
                    if (i % 2 == 0) firstList += tmpItemTemplate;
                    else secondList += tmpItemTemplate;
                }
        ...
        startCannvas: function(canvasId, elementsId) {
            TagCanvas.textFont =
                "Open Sans,  sans-serif, Trebuchet MS, Helvetica, Arial";
    ...
    policy: {
        ...
        display: function() {
            ...
            if (
                wc.data.jsonData.chapters[wc.interface.currentElement.chapterIndex]
                .audio != "undefined" &&
                wc.interface.actionHandler.setAudioState.isAudioOn
            ) {
    ...
    tableOfContents: {
        ...
        select: function(option) {
            ...
            wc.interface.generalDisplayMethods.selectListItem(
                "mediaBullet_toc_",
                option
            );
        ...
        display: function(options) {
            ...
            //wc.mediaPlayer.stopCurrentActiveMedia(true);
            //}
            wc.interface.displaySecondLayer(
                wc.interface.elementTypes.tableOfContents,
                null,
                options
            );
            ...
            if (
                typeof currentChapter.audio != "undefined" &&
                wc.interface.actionHandler.setAudioState.isAudioOn
            ) {
                wc.mediaPlayer.audio.pause();
                wc.interface.actionHandler.setAudioState.disableAudioState();
            ...
            //turn video off, if we have any
            if (
                typeof currentChapter.video != "undefined" &&
            ...
            if (
                typeof currentChapter.popQuiz != "undefined" &&
                currentChapter.popQuiz != null &&
                typeof currentChapter.popQuiz.video != "undefined" &&
                currentChapter.popQuiz.video != null
            ) {
    ...
    resizeFont: {
        ...
        resize: function() {
            ...
            if (
                wc.interface.elements.resizeFont.currentStep >=
                wc.interface.elements.resizeFont.maxSteps
            ) {
                increment = -1 *
                    (wc.interface.elements.resizeFont.maxSteps - 1) *
                    wc.interface.elements.resizeFont.fontSizeIncrement;
        ...
        reset: function() {
            ...
            currentFontSize -=
                increment * wc.interface.elements.resizeFont.currentStep;
            $("body").css("font-size", currentFontSize + "px");
            wc.interface.elements.resizeFont.currentStep = 0;
        }
    },
    quiz: {
        ...
        executeSelectedOption: function() {
            if (this.selectedOption != -1) {
                wc.interface.actionHandler.displayGameIntro(
                    wc.interface.elements.quiz.selectedChoiceIndex
                );
    ...
    fullTextButton: {
        ...
        isActive: false,
        activate: function() {
            var disableBulletsButton = document.getElementById(
                "Button_DisableBullets"
            );
            if (!isDefined(disableBulletsButton)) return;
    ...
    bulletsButton: {
        ...
        activate: function() {
            ...
            if (!isDefined(enableBulletsButton)) return;
    ...
    illustration: {
        inject: function(id, illustration, emptyContainer) {
            ...
            if (!isDefined(illustrationContainer)) return;
            if (!isDefined(illustration)) {
                illustrationContainer.innerHTML = "";
                return;
            }
            if (emptyContainer) illustrationContainer.innerHTML = "";
            ...
            if (illustration.variety == "application/x-shockwave-flash") {
                ...
                swfobject.embedSWF(
                    wc.interface.dataFilePath + illustration.src,
                    id,
                    illustration.width,
                    illustration.height,
                    "6",
    ...
    survey: {
        ...
        adjustSurveyResponse: function(responseIn) {
            ...
            for (i = 0; i < arrParams.length; i++) {
                ...
                if (i < arrParams.length - 1) outStr += "&amp;lt;";
            }
    ...
    mediaBullets: {
        ...
        getCuepointsForFlowPlayer: function(pithyQuote) {
            ...
            if (wc.interface.elements.mediaBullets.hasBullets(pithyQuote)) {
                ...
                if (
                    wc.interface.doBullets &&
                    isDefined(mediaBullets) &&
                    isDefined(mediaBullets.allBullets) &&
                    mediaBullets.allBullets.length > 0
                ) {
        ...
        parse: function(pithyQuote) {
            ...
            if (xmlDoc.childNodes.length > 0) {
                ...
                mediaBullets = {};
                mediaBullets.leftMargin = wc.tools.getXmlAttributeValue(
                    bulletsNode,
                    "leftMargin"
                );
                mediaBullets.topMargin = wc.tools.getXmlAttributeValue(
                    bulletsNode,
                    "topMargin"
                );
                mediaBullets.bulletToolVersion = wc.tools.getXmlAttributeValue(
                    bulletsNode,
                    "bulletToolVersion"
                );
                ...
                while (
                    bulletsNode.childNodes.length > 0 &&
                    bulletsNode.childNodes[0].nodeName.toLowerCase() != "bulletlist"
                ) {
                    bulletsNode = bulletsNode.childNodes[0];
                }

                // parse the bullets list
                mediaBullets.bulletsList = wc.interface.elements.mediaBullets.parseBulletList(
                    bulletsNode.childNodes[0],
                    null,
                    mediaBullets
                );
            }
        ...
        parseBulletList: function(xmlNode, bulletinList, mediaBullets) {
            try {
                ...
                if (typeof xmlNode != "undefined" && xmlNode.childNodes.length > 0) {
                    ...
                    for (
                        var nodeIndex = 0; nodeIndex < xmlNode.childNodes.length; nodeIndex++
                    ) {
                        ...
                        while (
                            ...
                            !(
                                childNode.nodeName.toLowerCase() == "bulletlist" ||
                                childNode.nodeName.toLowerCase() == "bullet"
                            )
                        ) {
                        ...
                        if (childNode.nodeName.toLowerCase() == "bullet") {
                            ...
                            try {
                                ...
                                bullet.content =
                                    childNode.innerHTML ||
                                    childNode.text ||
                                    childNode.firstChild.textContent ||
                                    childNode.firstChild.innerText ||
                                    childNode.firstChild.nodeValue ||
                                    childNode.firstChild.text;

                                // ie 7 and 8 parser issue
                                var stringnode = "";
                                for (
                                    var bulletnodeIndex = 0; bulletnodeIndex < childNode.childNodes.length; bulletnodeIndex++
                                ) {
                                    stringnode =
                                        stringnode + childNode.childNodes[bulletnodeIndex].xml;
                                }
                            ...
                            bulletinList.push(
                                wc.interface.elements.mediaBullets.parseBulletList(
                                    childNode,
                                    bulletinList,
                                    mediaBullets
                                )
        ...
        render: function(pithyQuote) {
            ...
            var mediaBulletinsWrapperTemplate =
                wc.data.templateElements.mediaBulletinsWrapper;
            ...
            mediaBulletinsWrapperTemplate = mediaBulletinsWrapperTemplate.replaceTag(
                "Content",
                "MarginLeft",
                mediaBullets.leftMargin
            );
            mediaBulletinsWrapperTemplate = mediaBulletinsWrapperTemplate.replaceTag(
                "Content",
                "MarginTop",
                mediaBullets.topMargin
            );
            wc.interface.elements.mediaBullets.mediaBulletIndex = 0;
            mediaBulletinsWrapperTemplate = mediaBulletinsWrapperTemplate.replaceTag(
                "Content",
                "BulletListContent",
                wc.interface.elements.mediaBullets.renderBulletList(
                    mediaBullets.bulletsList
                )
            );
        ...
        renderBulletList: function(bulletList) {
            ...
            var bulletListElementTemplate =
                bulletList.type == "ordered" ?
                wc.data.templateElements.mediaBulletinsOrderedList :
                wc.data.templateElements.mediaBulletinsUnorderedList;
            ...
            bulletListElementTemplate = bulletListElementTemplate.replaceTag(
                "Content",
                "Level",
                bulletList.level
            );
            bulletListElementTemplate = bulletListElementTemplate.replaceTag(
                "Content",
                "VertSpace",
                bulletList.vertSpace
            );

            var bulletListContent = "";
            for (
                var bulletIndex = 0; bulletIndex < bulletList.length; bulletIndex++
            ) {
                ...
                if (bulletList[bulletIndex] instanceof Array) {
                    listItemElementTemplate =
                        wc.data.templateElements.mediaBulletinsSublistWrapper;
                    //listItemElementTemplate = listItemElementTemplate.replaceTag('', );
                    listItemElementTemplate = listItemElementTemplate.replaceTag(
                        "Content",
                        "Level",
                        bulletList[bulletIndex].level
                    );
                    listItemElementTemplate = listItemElementTemplate.replaceTag(
                        "Content",
                        "BulletListContent",
                    ...
                    // get the template
                    listItemElementTemplate =
                        wc.data.templateElements.mediaBulletinsListItem;
                    // replace the values
                    listItemElementTemplate = listItemElementTemplate.replaceTag(
                        "Content",
                        "BulletIndex",
                    ...
                    listItemElementTemplate = listItemElementTemplate.replaceTag(
                        "Content",
                        "BulletContent",
                        bulletList[bulletIndex].content
                    );
                    wc.interface.elements.mediaBullets.mediaBulletIndex++;
                }
        ...
        showCuepoint: function(clip, cuepoint) {
            ...
            if (cuepoint.index > 0) {
                var previousBullet = document.getElementById(
                    "mediaBullet" + (cuepoint.index - 1)
                );
                previousBullet.className = previousBullet.className.replace(
                    /(?:^|\s)active(?!\S)/,
                    ""
                );
            ...
            mediaBullet.className = mediaBullet.className.replace(
                /(?:^|\s)hidden(?!\S)/,
                ""
            );
        ...
        resetActive: function() {
            ...
            var listHtmlElement = document.getElementById(
                "Chapter_Discussion_Content_MediaBulletsList"
            );
            if (isDefined(listHtmlElement)) {
                for (var listIndex in listHtmlElement.childNodes) {
                    try {
                        listHtmlElement.childNodes[
                            listIndex
                        ].className = listHtmlElement.childNodes[
                            listIndex
                        ].className.replace(/(?:^|\s)active(?!\S)/, "");
                    } catch (e) {}
                }
        ...
        toggleMainDiscussionOff: function() {
            ...
            if (
                isDefined(discussionContent) &&
                isDefined(mediaBulletins) &&
                wc.interface.elements.mediaBullets.currentDisplayMode == 1
            ) {
                discussionContent.style.display = "none";
                mediaBulletins.style.display = "";
            }
                    ...
                    switch (mediaBulletinsDisplayType) {
                        ...
                        case wc.interface.mediaBulletinsDisplayTypes.bullets:
                            ...
                            if (
                                wc.data.jsonData.chapters[
                                    wc.interface.currentElement.chapterIndex
                                ].chapterOptions.variety != "Wide"
                            )
                                discussionContent.style.display = "none";
                            else {
                                if (
                                    isDefined(document.getElementById("Chapter_Video_Container"))
                                ) {
                                    document.getElementById(
...
wc.interface.startChapterTiming = function() {
    ...
    if (isDefined(status)) status.innerHTML = status.innerHTML + "start ";
...
wc.interface.evaluateChapterTiming = function(curChapter) {
    if (0 == g_arrMinChapterTimes[curChapter - 1]) return false;
    else {
        ...
        return (
            wc.interface.currentChapterTime >=
            wc.interface.g_arrMinChapterTimes[curChapter - 1] * 1000
        );
    }
};
...
wc.interface.stopChapterTiming = function() {
    if (wc.interface.timingValid) {
        ...
        wc.interface.currentChapterTime +=
            wc.interface.endTime.getTime() - wc.interface.beginTime.getTime();
    }
...
wc.interface._applyScroll = function(element, manualRetry) {
    ...
    if (!isDefined(jQueryElement.data("scrollAttachAttempts")) || manualRetry) {
        ...
        jQueryElement.data(
            "scrollAttachAttempts",
            jQueryElement.data("scrollAttachAttempts") + 1
        );
    }
    ...
    if (!jQueryElement.is(":visible")) {
        ...
        var nextTimeout = 50;
        if (jQueryElement.data("scrollAttachAttempts") > 5) nextTimeout = 200;
        setTimeout(function() {
            wc.interface._applyScroll(element);
        }, nextTimeout);
        return;
    }
...
wc.interface.storage = {
    saveToStorage: function(key, value) {
        if (typeof localStorage == "undefined") this.setCookie(key, value, 100);
        else localStorage.setItem(key, value);
    },
    loadfromStorage: function(key) {
        if (typeof localStorage == "undefined") return this.getCookie(key);
...
wc.interface.generalDisplayMethods = {
    selectListItem: function(name, choiceIndex, doNotClearSelection) {
            ...
            while (elementLi) {
                if (isDefined(elementLi.className)) {
                    ...
                    if (
                        ...
                        isDefined($(elementLi).find("div")[0].className)
                    ) {
                        $(elementLi).find("div")[0].className = $(elementLi)
                            .find("div")[0]
        ...
        if (!isDefined(doNotClearSelection) || doNotClearSelection == false) {
            ...
            if (
                $(li).find("div").length > 0 &&
                isDefined($(li).find("div")[0].className)
            )
        ...
        if (isDefined(doNotClearSelection) && doNotClearSelection == true) {
            if (
                ...
                if ($(li).find("div")[0].className.indexOf("selected") >= 0)
                    $(li).find("div")[0].className = $(li)
                    .find("div")[0]
                    .className.replace("selected", "");
                else $(li).find("div")[0].className += "selected";
            }
        }
    }
};
wc.interface.customNavigation = {
    moveForward: function(numberOfChapters) {
        wc.interface.currentElement.chapterIndex =
            wc.interface.currentElement.chapterIndex + numberOfChapters - 1;
        wc.interface.navigateToElement(wc.interface.directions.next);
    },
    ...
    replaceTagsWithJavaScript: function(html) {
        ...
        htmlToReplace = htmlToReplace.replace(
            /<a href="navigate:moveForward\(([0-9]+)\);">/gi,
            '<a href="javascript:void(0);" id="navigate_moveForward_$1" onclick="wc.interface.customNavigation.moveForward($1);">'
        );
        htmlToReplace = htmlToReplace.replace(
            /<a href="navigate:moveBackward\(([0-9]+)\);">/gi,
            '<a href="javascript:void(0);" id="navigate_moveBackward_$1" onclick="wc.interface.customNavigation.moveBackwards($1);">'
        );
        htmlToReplace = htmlToReplace.replace(
            '<a href="navigate:exitChapters();">',
            '<a href="javascript:void(0);" id="navigate_exitChapters" onclick="wc.interface.navigateToElementAfterLastChapter();">'
        );
...
wc.interface.transition = {
    ...
    showModalWin_all: function() {
        ...
        if (
            typeof wc.data.jsonData.chapters[wc.interface.currentElement.chapterIndex]
            .video != "undefined" &&
            wc.data.jsonData.chapters[wc.interface.currentElement.chapterIndex]
            .video != null
        ) {
        ...
        if (
            ...
            $("<div/>")
                ...
                .fadeTo("600", 0.7, function() {
                    ...
                    $("#SecondLayerElementContainer").addClass(
                        "LayerElementContainerCustomized"
                    );
                    if (wc.interface.options.is508) {
                        $("#CurrentElementContainer").find("*[tabindex]").each(function() {
            ...
            $("<div/>")
                ...
                .fadeTo("600", 0.7, function() {
                    ...
                    $("#SecondLayerElementContainer").show(
                        "slide", { direction: "left" },
                        600
                    );
                    if (wc.interface.options.is508) {
    ...
    hideModalWin_all: function(resumeAudio, data) {
        ...
        elementType = wc.interface.currentElement.elementType;
        if (data != null && typeof data != "undefined") ordinal = data.ordinal;
        ...
        if (
            ...
            $(LayerElementContainer).slideUp(600, function() {
                ...
                if (wc.interface.options.is508) {
                    ...
                    setTimeout(function() {
                        ...
                        if (
                            elementType != wc.interface.elementTypes.term &&
                            elementType != wc.interface.elementTypes.linkBulletin
                        ) {
                            var element = $("#CurrentElementContainer")
                                .find("*[tabindex]")
            ...
            $(LayerElementContainer).hide(
                "slide", { direction: "left" },
                300,
                function() {
                    $("body div.modal-overlay").remove();
                    $("body div.modal-overlay-aux").remove();
                    $(LayerElementContainer).removeClass(
                        "LayerElementContainerCustomized"
                    );
                    if (wc.interface.options.is508) {
                        $("#CurrentElementContainer").find("*[tabindex]").each(function() {
    ...
    hideModalWin_allSecondLayer: function() {
        ...
        if (
            ...
            $(SecondLayerElementContainer).hide(
                "slide", { direction: "left" },
                600,
                function() {
                    $("body div.modal-overlay").remove();
...
function getLanguageCode(id) {
    ...
    var languages = [
        //Used the id, codes and languages from the admin languages page
