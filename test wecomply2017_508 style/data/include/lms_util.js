...
if(typeof(AICCResponse.LESSON_LOCATION_VARCHAR) == 'undefined' || AICCResponse.LESSON_LOCATION_VARCHAR == '')
{
    objInfo.lesson_location_varchar = '';
}
else
{
    objInfo.lesson_location_varchar = LESSON_LOCATION_VARCHAR;
}
	...
	if (!bHACP)
	{
		...
		if (lmsAPI != null)
		{
			...
			else
			{
                ...
                //objInfo.lesson_location_varchar = lmsApiGetValue(lmsAPI, "cmi.core.lesson_location_varchar");
				//objInfo.lesson_location_varchar = "lesson_location_varchar";
...
function lmsSendInfo(objInfo, bFinish)
{
    ...
    // AAAAAA
    if (objInfo.lesson_location_varchar)
        lesson_location_varchar = objInfo.lesson_location_varchar;
    else
        lesson_location_varchar = '';
...
var strAICCTime = getElapsedTime(objInfo.startTime);
	...
	if (objInfo.aicc_url)
	{
		...
		// Send info to LMS
		//debug("Sending AICC data: score=" + aiccScore + "; time=" + strAICCTime + "; status=" + status + "; lesson_location=" + lesson_location + (suspend_data != null ? "; lesson_location_varchar=" + lesson_location_varchar + ";suspend_data=" + objInfo.suspend_data : ''));
        	if (lesson_location_varchar != null && lesson_location_varchar != '') { 
			debug("Sending AICC data: score=" + aiccScore + "; time=" + strAICCTime + "; status=" + status + "; lesson_location=" + lesson_location + (suspend_data != null ? ";suspend_data=" + objInfo.suspend_data : '') + "; lesson_location_varchar=" + lesson_location_varchar + ";");
		} else {
		...
		strAICCResult = AICCSendScore(strAICCTime, aiccScore, status, lesson_location, aicc_url, aicc_sid, suspend_data, lesson_location_varchar);
...
// new code to deal with denso (need to strip leading carriage returns) 
		if (nResult == 999) {
		  strAICCResult = strAICCResult.replace(/^\s+/,"");
		  if (strAICCResult != "") {
		    var err = strAICCResult.split("\r\n", 1);
		    if (err != null && typeof (err) == "object" && err.length == 1) {
			        var errNum = err[0].split("=");
...
// new code deal with intellum (unix carriage returns
		if (nResult == 999) {
		  strAICCResult = strAICCResult.replace(/^\s+/,"");
		  if (strAICCResult != "") {
		    var err = strAICCResult.split("\n", 1);
  ...
  //      lmsApi.LMSSetValue("cmi.core.lesson_location_varchar", lesson_location_varchar);
			debug("Setting SCORM values: score=" + score + "; score.max=" + scoreMax + "; score.min=" + scoreMin + "; time=" + strAICCTime + "; status=" + status + "; lesson_location=" + lesson_location + "; lesson_location_varchar=" + lesson_location_varchar);
