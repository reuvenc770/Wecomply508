...
//alert("In video play");
			if (wc.mediaPlayer.support.html5Video(true)) {
//alert("detecting that I should be able to support html5");
			} else {
//alert("detecting no support for html5, currently trying to force html so that is why this is freezing.  ");
			}
			if (wc.mediaPlayer.support.html5Video(true))
//			if (1==1) 
			{
//alert("forcing html5");
		...
		//		wc.mediaPlayer.video.activeElement.src = wc.data.jsonData.chapters[chapterIndex].video.src.replace('.flv', '.mp4');
				...
				{
			var continueButton = document.getElementById("PopQuiz_ContinueBtton");
			...
			else if (wc.mediaPlayer.support.flashVideo())
			{
...
//alert("in flash player, enabling continue button immediately");
				$(popquizcontinueButton).removeClass('disabled');
				wc.interface.actionHandler.helpers.restoreDefaultFunctionality(popquizcontinueButton);
			}
...
wc.mediaPlayer.controls.autoUpdate = function() {
	try {
		...
		wc.mediaPlayer.controls.update(parseInt(progress));
         ...
         // this doesn't work when going back/replaing video in pop quiz
	//	alert(e);
	}
};
