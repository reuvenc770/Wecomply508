        ...
        if (canDoHACP(aicc_url)) {
            ...
            if (typeof (strData) == "undefined" || strData == "") {
                ...
                if (typeof (strStudentID) == "undefined" || strStudentID == "") {
                    ...
                    // ATTN: For "regular" lessons the if after this line used to be the correct way.
                    //this.cmi.core.lesson_location = AICCResponse.LESSON_LOCATION;
                    this.cmi.core.lesson_location = AICCResponse.LESSON_LOCATION_VARCHAR;
                    ...
                    */
                    ...
                    // create the _children object. DOC: From what I gather, the list of items inside the single quotes represents the values supported by our object. If, for example, lesson_location is not part of the list then the lesson package will not query it!
                    this.cmi.core._children = 'student_id, student_name, lesson_location, credit, lesson_status, entry, score, total_time, lesson_mode, exit, session_time';
                }
                AICCResponse = null;
            }
        }
...
API_1_2_WRAPPER.GetValue = function (key) {
	...
	else if (key.indexOf("cmi.objectives.") == 0) {
...
if (value === undefined) {
  value = '';
	...
	if (key.indexOf("cmi.suspend_data") == 0) {
// strip carriage returns out of suspend_data
	value = value.replace(/(\r\n|\n|\r)/gm,"");
	}
...
API_1_2_WRAPPER.SetValue = function (key, passedvalue) {

if (passedvalue != null) {
value = passedvalue.toString();
} else {
value = "";
}
...
//value = value.replace(/\r/g,"");
	...
	for (var i = 0; i < keyObjects.length - 1; i++) {
		...
		try {
			var x = eval('this.cmi');
			if (typeof (x) == 'undefined')
				eval('this.cmi = new Object()');
		} catch (e) {

		}
		try {
			var x = eval('this.cmi.core');
			if (typeof (x) == 'undefined')
				eval('this.cmi.core = new Object()');
		} catch (e) {

		}
		try {
			var x = eval('this.cmi.core.lesson_status');
			if (typeof (x) == 'undefined')
				eval('this.cmi.core.lesson_status = new Object()');
		} catch (e) {

		}
		try {
			var x = eval('this.cmi.core.lesson_location');
			if (typeof (x) == 'undefined')
				eval('this.cmi.core.lesson_location = new Object()');
		} catch (e) {

		}
		try {
			var x = eval('this.cmi.core.score');
			if (typeof (x) == 'undefined')
				eval('this.cmi.core.score = new Object()');
		} catch (e) {

		}


//special code for weatherford, to look for score, location, and status within suspend_data
if (key == "cmi.suspend_data") {
  debug ("got suspend data " + value);

  if (value.indexOf("lesson_location=") >= 0) {





  var suspendobjects = value.split(/\s/);

  debug("number of objects " + suspendobjects.length);
  debug("1 first object " + suspendobjects[0]);

	for (var i = 0; i < suspendobjects.length - 1; i++) {
           debug("got suspend object " + suspendobjects[i]);
           if (suspendobjects[i].indexOf("lesson_location=") >= 0) {
             mylocation = suspendobjects[i].split("=");
             this.cmi.core.lesson_location = mylocation[1];

             debug("got location " + this.cmi.core.lesson_location);
           }
           if (suspendobjects[i].indexOf("lesson_status=") >= 0) {
             mystatus = suspendobjects[i].split("=");
             this.cmi.core.lesson_status = mystatus[1];

             debug("got status " + this.cmi.core.lesson_status);
           }
           if (suspendobjects[i].indexOf("score=") >= 0) {
             myscore = suspendobjects[i].split("=");
             this.cmi.core.score = myscore[1];
             debug("got score " + this.cmi.core.score);
           }
	}
...
//end weatherford code
try {
var setstring = 'this.' + key + ' = ' + '"' + value + '"';
debug('in API_1_2_WRAPPER.SetValue Value ' + setstring);
eval(setstring);
} catch (e) {
debug('normal eval throws error, try something else');
try {
var escapevalue = value;
escapevalue = escapevalue.replace(/"/g,'\\\"');
setstring = 'this.' + key + ' = ' + '"' + escapevalue + '"';
debug('in API_1_2_WRAPPER.SetValue Value ' + setstring);
eval(setstring);
} catch (e2) {

debug('still throwing error ' + e2);
}
//return true;
}
	return true;
...
API_1_2_WRAPPER.Commit = function (key) {
	...
	var nresult = lmsSendInfo(this.GetAICCObject(), false);
	debug('nresult: ' + nresult);
	if (nresult == 0) {
	return true;
	} else {
	return false;
...
API_1_2_WRAPPER.GetAICCObject = function () {
	...
	objInfo.bookmark = this.cmi.core.lesson_location;
    objInfo.lesson_location_varchar = this.cmi.core.lesson_location; // send the same value as .bookmark. This one is handled correctly by the backend and the other value is there for backwards compatibility
    //debug('Bookmark value: ' + objInfo.bookmark);
		...
		if (canDoHACP(aicc_url)) {
			...
			if (typeof (strData) == "undefined" || strData == "") {
				...
				if (typeof (strStudentID) == "undefined" || strStudentID == "") {
					...
					this.cmi.location = AICCResponse.LESSON_LOCATION_VARCHAR;
					this.cmi.suspend_data = "";
					if (typeof(AICCResponse.SUSPEND_DATA) != 'undefined')
						this.cmi.suspend_data = AICCResponse.SUSPEND_DATA;
...
API_2004_WRAPPER.SetValue = function (key, passedvalue) {
        var value = passedvalue.toString();
	debug('Setting ' + key + '=' + value);
...
function getQsParam(paramName) {
	...
	if ((window.frames != null)
		...
		params = window.frames[0].location.search.substring(1).split('&');
