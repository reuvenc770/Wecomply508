	...
	if (!wc.WindowLoaded || typeof (wc.lms.window) != "object" || wc.lms.window.bReady != true) {
		...
		setTimeout("wc.init()", 100);
			...
			if (wc.interface.allowFlashInterface && wc.interface.type == wc.interface.types.flash) {
				...
				if (typeof(page) != 'undefined' && page != "" && parseInt(page) > 0) {

					return wc.interface.replaceCurrentElement(wc.interface.elementTypes.chapter, parseInt(page) - 1);
				}
...
wc.interface.replaceCurrentElement = function (nextElementType, nextChapterIndex) {
	...
	var buttonsToHighlight = wc.interface.navigation.buttonTypes.None;
	var buttonsToUnHighlight = wc.interface.navigation.buttonTypes.None;
	...
	if (nextElementType == wc.interface.elementTypes.chapter) {
		...
		if (nextChapter.chapterOptions.variety == 'Memorandum') {
			...
			templateHtml = templateHtml.replaceTag('Content', 'Chapter_Keywords', nextChapter.keyword);
			// if audio is enabled then display bulletins otherwise show discussion
			var chapterMediaBulletsListHtml = '';
	...
	wc.interface.navigation.updateButtons(buttonsToEnable, buttonsToDisable, buttonsToHide, buttonsToActivate, buttonsToHighlight, buttonsToUnHighlight);
	wc.interface.currentElement.displayType = wc.interface.displayTypes.Container;
};
		...
		if (typeof (closeButton) != 'undefined' && closeButton != null) {
			closeButton.onclick = function () {
				...
				currentElementopacityTween.onMotionFinished = function () {
					...
					wc.interface.navigation.updateButtons(buttonsCurrentState.buttonsToEnable, buttonsCurrentState.buttonsToDisable, buttonsCurrentState.buttonsToHide, buttonsCurrentState.buttonsToActivate, buttonsCurrentState.buttonsToHighlight, buttonsCurrentState.buttonsToUnHighlight);
				};
				currentElementopacityTween.start();
				wc.interface.currentElement.displayType = wc.interface.displayTypes.Container;
...
wc.interface.displaySecondLayer = function(elementType) {
	...
	else if (elementType == wc.interface.elementTypes.mode) {
		templateHtml = modedata;
	}
...
//alert('trying to stop in navigatetoelement');
        wc.mediaPlayer.video.stop();
//alert('done trying to stop video in navigatetoelement');
...
//alert('navigateToElement:{direction:' + direction + ',nextElementType:' + nextElementType + ',nextChapterIndex:' + nextChapterIndex + '}');

	// report to lms
	if (wc.lms.doBookmarkUpdate) {
		if (!wc.lms.updateBookmark(name)) {
		alert('failed updating bookmark');
...
//alert("replace element type " + nextElementType + " chapter " + nextChapterIndex);
...
wc.interface.elements = {
	...
	mode:
	{
		buttonId: 'Button_modetool',
		display: function () {
			wc.interface.displaySecondLayer(wc.interface.elementTypes.mode);
		},
		close: function () {
			wc.interface.closeSecondLayer();
		}
	...
	mediaBullets: {
		...
		parse: function (pithyQuote) {
			...
			if (xmlDoc.childNodes.length > 0) {
				...
				mediaBullets.leftMargin = bulletsNode.getAttribute("leftMargin");
				mediaBullets.topMargin = bulletsNode.getAttribute("topMargin");
				mediaBullets.bulletToolVersion = bulletsNode.getAttribute("bulletToolVersion");
		...
		parseBulletList: function (xmlNode, bulletinList, mediaBullets) {
			...
			if (xmlNode.childNodes.length > 0) {
				...
				bulletinList = [];
				bulletinList.vertSpace = wc.tools.getXmlAttributeValue(xmlNode, 'vertSpace');
				bulletinList.level = wc.tools.getXmlAttributeValue(xmlNode, 'level');
				bulletinList.type = wc.tools.getXmlAttributeValue(xmlNode, 'type');
				...
				for (var nodeIndex = 0; nodeIndex < xmlNode.childNodes.length; nodeIndex++) {
					...
					if (childNode.nodeName.toLowerCase() == 'bullet') {
						var bullet = {};
						bullet.size = wc.tools.getXmlAttributeValue(childNode, 'size');
						bullet.time = wc.tools.getXmlAttributeValue(childNode, 'time');
						bullet.color = wc.tools.getXmlAttributeValue(childNode, 'color');
						bullet.content = childNode.innerHTML;

						bullet.content = childNode.firstChild.textContent||childNode.firstChild.innerText||childNode.firstChild.nodeValue;
						if (bullet.content == 'undefined') {
							alert("bullet content undefined");
						}
//alert("bullet time " + childNode.getAttribute('time'));
						bullet.size = childNode.getAttribute('size');
						bullet.time = childNode.getAttribute('time');
						bullet.color = childNode.getAttribute('color');

//						if (bullet.content == 'undefined') {
//alert("bullet content" + bullet.content + " node name " + childNode.nodeName + " node type " + childNode.nodeType + " node value " + childNode.nodeValue + " child node type " + childNode.firstChild.nodeType + " child node text value " + childNode.firstChild.nodeValue );
//alert("bullet content " + bullet.content + " time " + bullet.time + " outerhtml " + childNode.outerHTML + " parent inner html " + childNode.parentNode.innerHTML + " parent inner text " + childNode.parentNode.innerText);
//						}
						try {
					...
					//	if (bullet.content == 'undefined') {
					//		bullet.content = childNode.innerText;
					//	}
						//var tmp = document.createElement("div");
						//tmp.appendChild(childNode);
						//bullet.content = tmp.innerHTML;
// not sure why childNode.innerHTML doesn't work
//alert('bullet content ' + bullet.content + ' bullet time ' + bullet.time + ' outer ' + tmp.innerHTML);
...
wc.interface.actionHandler = {
	...
	quizQuestion_MultipleChoiceOneAnswerSelected: function (choiceIndex, elementType) {
		...
		if (isCorrect == false) {
			...
			if (isDefined(wc.data.values.elementsName) && wc.interface.currentElement.elementType == wc.interface.elementTypes.popQuestion && (wc.data.values.elementsName == 'iPadFrame' || wc.data.values.elementsName == 'WeComply2013' || wc.data.values.elementsName == 'iPadFrameAccenture5' || wc.data.values.elementsName == 'iPadFrameAccenture4')) {
				if (isDefined(tryAgainElement)) {
