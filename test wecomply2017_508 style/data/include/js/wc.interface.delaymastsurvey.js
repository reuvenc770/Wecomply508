...
wc.interface.replaceCurrentElement = function (nextElementType, nextChapterIndex) {
	...
	if (nextElementType == wc.interface.elementTypes.chapter) {
		...
		currentElementContainer.innerHTML = templateHtml;
			...
			if (typeof (nextChapter.survey) != 'undefined') {
			document.getElementById('chalkboard').style.width="817px";

			document.getElementById('corkboard').style.width="40px";
			}
		...
		if (cpic) {
			var pagechapter = nextChapterIndex + 1;
			var pagetotal = 1;
                        if (nextChapterIndex + 1 > 1) {
			
			pagechapter = nextChapterIndex + 1 - 1;
			pagetotal = 22;
			}

			cpic.innerHTML = document.getElementById('Chapter_PageInfo_Content').innerHTML.replace('$1page', pagechapter).replace('$2page', pagetotal);
...
wc.interface.game = {
	...
	init: function (gameIndex) {
		...
		//questionChapters.sort(wc.tools.randomizeSort);
               if (wc.interface.options.games.randomizeQuestions == "false") {
		} else {
...
// for pseg, move on to next question regardless of true or false
...
//						wc.interface.game.currentQuestions = wc.tools.shuffle(wc.interface.game.currentQuestions);
					...
					{
// done with quiz, go to final quiz
						wc.interface.game.currentQuestions = null;
						wc.interface.game.chapterQuestionsCompleted = true;
					}
//				}
...
//alert("navigate to next element after game");
//wc.interface.replaceCurrentElement(wc.interface.elementTypes.chapter, wc.interface.currentElement.chapterIndex + 1);

			//wc.interface.navigateToElement(wc.interface.directions.next);
		} else {

			var choiceHtml = '';
			for (var choiceIndex in (!wc.interface.game.chapterQuestionsCompleted && wc.interface.options.games.randomizeChoices ? question.choices : question.choices)) {
		...
		//Pagination, has to be done after the pagination is part of the DOM
		//var cpic = document.getElementById('Chapter_PageInfo_Content');
		//if (cpic && elementType == wc.interface.elementTypes.game) {
		//	var pagetotal = 10;
			templateHtml = templateHtml.replace('$1page', wc.interface.game.questionsAskedPoolCount+1 );

			templateHtml = templateHtml.replace('$2page', '15' );
		//}
...
//alert("navigate to next element after game end of if");
//wc.interface.replaceCurrentElement(wc.interface.elementTypes.chapter, wc.interface.currentElement.chapterIndex);
//alert("after update buttons in end game");
wc.interface.replaceCurrentElement(wc.interface.elementTypes.chapter, wc.interface.currentElement.chapterIndex + 1);
	    buttonsToDisable ^= wc.interface.navigation.buttonTypes.Help;
	    buttonsToDisable ^= wc.interface.navigation.buttonTypes.NextChapter;

	   buttonsToEnable |= wc.interface.navigation.buttonTypes.Help;
...
//alert("in navigate, nextchapterindex " + wc.interface.currentElement.chapterIndex + " next type " + wc.interface.currentElement.elementType);

//pseg tweak
if (wc.interface.currentElement.elementType == wc.interface.elementTypes.game) {
wc.interface.currentElement.elementType = wc.interface.elementTypes.chapter;
nextChapterIndex = wc.interface.currentElement.chapterIndex;
wc.interface.replaceCurrentElement(wc.interface.elementTypes.chapter, wc.interface.currentElement.chapterIndex + 1);
//alert("after game replace");
}

//alert("type " + wc.interface.currentElement.elementType + " chapter " + wc.interface.currentElement.chapterIndex);
...
//pseg tweak
// if chapter = 55 then navigatetoelementafterlastchapter
// if chapterindex is last page, then go to cert


						if (wc.interface.currentElement.chapterIndex >= wc.data.jsonData.chapters.length - 1) {
//							nextElementType = wc.interface.navigateToElementAfterLastChapter();
//display cert here
//alert("at end of course, displaying certificate");
	return wc.interface.actionHandler.displayCertificate();
						} else if (wc.interface.currentElement.chapterIndex == 0) {

//alert("before last chapterIndex " + wc.interface.currentElement.chapterIndex + " nextChapterIndex " + wc.interface.currentElement.nextChapterIndex);

						//	wc.interface.currentElement.chapterIndex = wc.interface.currentElement.chapterIndex + 1 ;

						//	wc.interface.currentElement.nextChapterIndex = wc.interface.currentElement.chapterIndex + 1;
							nextChapterIndex = wc.interface.currentElement.chapterIndex + 1;
							nextElementType = wc.interface.navigateToElementAfterLastChapter();

//alert("after last chapterIndex " + wc.interface.currentElement.chapterIndex + " nextChapterIndex " + wc.interface.currentElement.nextChapterIndex);
						} else {
							nextChapterIndex = wc.interface.currentElement.chapterIndex + 1;
							nextElementType = wc.interface.elementTypes.chapter;
						}
...
//alert("in navigatetoalementafterlast display certificate");
		return wc.interface.actionHandler.displayCertificate();
	}
...
//alert("in general navigatetolast displaycert");
	return wc.interface.actionHandler.displayCertificate();
...
//console.log("bullet parsing innerhtml " + tmp.innerHTML);
...
// new, unselect any other choice
		var survey = wc.data.jsonData.chapters[wc.interface.currentElement.chapterIndex].survey;
		if (isDefined(survey.pivotQuestion)) {
			for (var mychoiceIndex in survey.pivotQuestion.choices) {
				if (choiceIndex != mychoiceIndex) {
                                 document.getElementById('Survey_PivotQuestion_' + mychoiceIndex).checked = false;
...
//alert("To print the certificate, please set your printer settings to landscape mode. This is not mandatory, and your results will still be recorded and reported in the system.");
alert("You have just completed COURSE 2. Thank you!  \n\nYour responses to the COURSE 2 quiz and compliance questionnaire are recorded in the system. Any changes to these responses will not be recorded. If you wish to revise your responses to the compliance questionnaire, please contact the Office of Ethics and Integrity at 973-430-6405. \n\nWhen you advance to the next page you will see the following:\n\n • The option to print your recorded responses to the compliance questionnaire.\n\n • Your COURSE 2 completion certificate.  If you wish to print it, please adjust your printer settings to landscape mode.");
