...
MediaPlayer.controls.setAutoUpdate = function(autoElement, startProgress){
    ...
    {
        MediaPlayer.controls.intval = self.setInterval(MediaPlayer.controls.autoUpdate, 500);
    }
    else{
...
MediaPlayer.controls.update = function (progress){
   ...
   // var progressBar = document.getElementById('mediaPlayerControlsProgressBar');
    ...
    var newProgress = (progressBarWidth / 100) * progress;
    if (seekerBarWidth > newProgress)
        seekerBarWidth = newProgress;
    for (var x = seekerBarWidth; x < newProgress;x++)  
        seekerBar.style.width = x + 'px';
	...
	else
	{
		if (typeof(MediaPlayer.audio.files[id]) != 'undefined'){
			...
			if (MediaPlayer.support.html5Audio(true)){
				...
				MediaPlayer.audio.activeElement.src = MediaPlayer.audio.files[id].src;
				...
				var checkFlash = function(){
					...
					{
						MediaPlayer.audio.activeElement.SetVariable('currentSong', 'http://test.wecomply.com' + MediaPlayer.audio.files[id].src);
