...
wc.interface.displayLayer = function (elementType, data) {
	...
	if (elementType == wc.interface.elementTypes.game) {
		...
		//};
			...
			document.getElementById('LayerElementContainer').style.opacity = '100';
		

			if (document.all)	// IE
			{
				document.getElementById('LayerElementContainer').style.filter = 'alpha(opacity=99)';
				document.getElementById('LayerElementContainer').style.filter = 'alpha(opacity=100)';
			}
		//currentElementopacityTween.start();
		wc.interface.actionHandler.scrollToTop();
		//wc.interface.elements.quiz.resizeFont(true);
...
// not sure why I have to do this here, should have already been done elsewhere, but staples needs it to show feedback in ie
			if (document.all)	// IE
			{
				document.getElementById('LayerElementContainer').style.filter = 'alpha(opacity=100)';
				document.getElementById('LayerElementContainer').style.filter = 'alpha(opacity=99)';
...
//alert('new choice is trigger ' + choice.isTrigger);
//hasclass
var pivotli = document.getElementById('survey_pivot_mediaBullet_' + choiceIndex);
if ($(pivotli).hasClass('hidden') ) {
//alert('is hidden true');
} else {
//alert('is hidden false');
}
...
//alert('checked ' + document.getElementById('Survey_PivotQuestion_' + choiceIndex).checked);
...
//alert('triggerchecked ' + isTriggerCheckedOrSkipped);
