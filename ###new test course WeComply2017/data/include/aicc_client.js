...
//
function AICCSendScore(strTime, strScore, strStatus, strLocation, strAICCURL, strAICCSessionID, strSuspendData, strLessonLocationVarchar)
{
	var strAICCData;
    ...
    //don't send lesson_location_varchar if it is empty  (really, lesson_location_varchar should probably be under vendor, not sure if shulman course will deal with that
        if (strLessonLocationVarchar != null && strLessonLocationVarchar != '') { 
	strAICCData = "[CORE]\r\nlesson_location=" + strLocation + "\r\nlesson_location_varchar=" + strLessonLocationVarchar + "\r\nlesson_status=" + strStatus + "\r\nTime=" + strTime + "\r\nscore=" + strScore + "\r\n" + (strSuspendData != 'undefined' && strSuspendData != null ? "suspend_data=" + strSuspendData + "\r\n" : "");
	} else {
	strAICCData = "[CORE]\r\nlesson_location=" + strLocation + "\r\nlesson_status=" + strStatus + "\r\nTime=" + strTime + "\r\nscore=" + strScore + "\r\n" + (strSuspendData != 'undefined' && strSuspendData != null ? "suspend_data=" + strSuspendData + "\r\n" : "");
	}

	//debug('Lesson location varchar: ' + strLessonLocationVarchar);
	//console.log(strLessonLocationVarchar);
