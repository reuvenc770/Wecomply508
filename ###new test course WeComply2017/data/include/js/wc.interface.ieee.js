...
// for apple mobile, enable button for ieee on first chapter to trigger audio
//		if (wc.deviceDetection.isApple() && wc.lms.highestBookmark==0) {
		if (wc.lms.highestBookmark==0) {
		var startfirstchapter = document.getElementById('MobileStartFirstChapter');
		if (isDefined(startfirstchapter) && nextChapterIndex == 0) {
			startfirstchapter.style.display="";
		}
		}
...
if(wc.interface.actionHandler.setAudioState.isAudioOn) {
...
wc.interface.actionHandler = {
	...
	closeMobileFirstChapter: function() {
//alert("hi");
		var startfirstchapter = document.getElementById('MobileStartFirstChapter');
		if (isDefined(startfirstchapter) ) {
			startfirstchapter.style.display="none";
		}
	},
	...
	popQuiz_answerSelected: function (answerIndex, questionIndex) {
		...
		if (choice.isCorrect == "true") {
			...
			tryAgainElement.innerHTML = wc.interface.getResource('Pleasetryagain');
			wc.interface.resizeScrolls();
//clear selection for ieee
//alert('clear quiz selection ' + questionIndex + answerIndex);
//   var ele = document.getElementsByName("popQuiz_" + questionIndex + "_" + answerIndex);
//   for(var i=0;i<ele.length;i++) {
//      ele[i].checked = false;
//alert('clear a selection');
//   }
			wc.interface.elements.quiz.clearSelection();
		}
...
//alert('clear quiz selection ' + questionIndex + answerIndex);
   ...
   for(var i=0;i<ele.length;i++) {
//alert("got tag" + ele[i].getAttribute("name") + " id " + ele[i].id);
      if(ele[i].getAttribute("name") == "popQuiz_" + questionIndex + "_" + answerIndex) {
	} else {
      ele[i].checked = false;
if (agreeElement.className=="agreeText") {
ele[i].disabled = true;
}
	}
//alert('clear a selection');
