...
wc.interface = {
	...
	elementTypes: {
		none: 0, intro: 1, chapter: 2, term: 3, linkBulletin: 4, popQuestion: 5, finalQuizQuestion: 6, acknowledgment: 7, survey: 8, certificate: 9, bookmark: 10, gameChoice: 11, game: 12, gameIntro: 13, popBulletin: 14, popQuiz: 15, matchGame: 16, note: 17, tableOfContents: 18, help: 19, policy: 20, gameRestart: 21, Congratulations: 22
	},
	introTypes: {
		none: 1, textSphere: 2
	},
...
//alert("in game restart");
// orix
templateHtml = templateHtml.replace('{Resource:GameRestartBody}', wc.interface.getResource('MinimumScoreWarning'));  
   
templateHtml = templateHtml.replace('{Resource:GameRestartHeader}', ''); 

templateHtml = templateHtml.replace('{Resource:GameRestart_TakeQuiz}', wc.interface.getResource('Continue'));

		var percent = Math.round((wc.interface.game.questionsCorrectOnFirstTry * 100) / wc.interface.game.questionCounter);
		templateHtml = templateHtml.replace('%1%', wc.data.jsonData.minimumPassingScore);
		templateHtml = templateHtml.replace('%2%', percent);
		templateHtml = wc.interface.customNavigation.replaceTagsWithJavaScript(templateHtml);
	...
	else if (elementType == wc.interface.elementTypes.Congratulations) {
		//var templateHtml = wc.data.templateElements.Congratulations;
//var templateHtml = "<div id=\"Congratulations_Element\" class=\"standardChapter\"><div class=\"smallFrame wc-acknowledgment-content-wrapper \">	<div class=\"gameRestartHeader chapterHeading wc-page-title wc-horiz-top-bar \">		<h3 id=\"GameRestart_Heading_Content\" class=\"gameRestartTitle\"></h3>	</div>					<div class=\"gameRestartBody\">		<div id=\"GameRestart_Body_Content\" class=\"gameRestartText courseContent\">{$Resource:Congratulations}</div>		<div class=\"gameRestartButtons\">							<div id=\"Congratulations_Continue\" class=\"closeButton firstButton continueButton wc-rectangular-button \" onclick=\"wc.interface.actionHandler.displayCertificate();\">{$Resource:Continue}</div>								</div>			</div>	</div></div>";

var templateHtml = "<div id=\"Congratulations_Element\" class=\"standardChapter\"><div class=\"smallFrame wc-acknowledgment-content-wrapper \">	<div class=\"gameRestartHeader chapterHeading wc-page-title wc-horiz-top-bar \">		<h3 id=\"GameRestart_Heading_Content\" class=\"gameRestartTitle\"></h3>	</div>					<div class=\"gameRestartBody\">		<div id=\"GameRestart_Body_Content\" class=\"gameRestartText courseContent\">{$Resource:Congratulations}</div>		<div class=\"gameRestartButtons\">							<div id=\"Congratulations_Continue\" class=\"closeButton firstButton continueButton wc-rectangular-button \" onclick=\"wc.interface.actionHandler.displayCertificate();\">{$Resource:Continue}</div>								</div>			</div>	</div></div>";

templateHtml = templateHtml.replace('{$Resource:Congratulations}', wc.interface.getResource('Congratulations')); 

templateHtml = templateHtml.replace('{$Resource:Continue}', wc.interface.getResource('Continue'));


//alert("in congratulations template " + templateHtml);
		...
		currentElementopacityTween.start();
		wc.interface.currentElement.elementType = wc.interface.elementTypes.Congratulations;
	}
...
wc.interface.displaySecondLayer = function (elementType, onDoneCallback) {
	...
	if (elementType == wc.interface.elementTypes.help) {
		...
		for (var i = 0; i < wc.data.jsonData.chapters.length; i++) {
			...
			if (i <= wc.interface.currentElement.chapterIndex || (wc.interface.isAdmin && i<=5) || i < wc.lms.highestPage || i < wc.lms.highestBookmark) {
...
//alert("in navigatetoelementafterlastchapter type " + wc.interface.currentElement.elementType);
...
//	if (wc.interface.currentElement.elementType == wc.interface.elementTypes.finalQuizQuestion) {
//alert("questioncounter " + wc.interface.game.questionCounter + " firsttry " + wc.interface.game.questionsCorrectOnFirstTry);
...
//	return wc.interface.actionHandler.displayCertificate();
        return wc.interface.actionHandler.displayCongratulations();
...
alert('choice.feedback is ' + choice.feedback);
	...
	displayCongratulations: function () {
		return wc.interface.displayLayer(wc.interface.elementTypes.Congratulations);
	},
