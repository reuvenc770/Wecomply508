...
wc.interface = {
	...
	elementTypes: {
		none: 0, intro: 1, chapter: 2, term: 3, linkBulletin: 4, popQuestion: 5, finalQuizQuestion: 6, acknowledgment: 7, survey: 8, certificate: 9, bookmark: 10, gameChoice: 11, game: 12, gameIntro: 13, popBulletin: 14, popQuiz: 15, matchGame: 16, note: 17, tableOfContents: 18, help: 19, policy: 20
	},
	introTypes: {
		none: 1, textSphere: 2
	},
	linkBulletinTypes: {
		Code: { id: 1, varietyCode: 'Code', varietyName: 'Code' },
		News: { id: 2, varietyCode: 'News', varietyName: 'News' },
		FAQs: { id: 3, varietyCode: 'FAQs', varietyName: 'FAQs' },
		Policy: { id: 4, varietyCode: 'Policy', varietyName: 'Policy' },
		DosAndDonts: { id: 5, varietyCode: 'DosAndDonts', varietyName: "Do's And Dont's" },
		CodeCompanion: { id: 6, varietyCode: 'CodeCompanion', varietyName: 'Code Companion' }
	},
...
wc.interface.loadGameTemplates = function (variety) {
	if (wc.interface.gameFilePath == '/Wc2/training/GameData.aspx') {
		...
		if (isDefined(wc.data.values.IsiPadFrame) && wc.data.values.IsiPadFrame == 'True') {
			path = '/Wc2/training/GameData.aspx?templatePlatform=' + 'iPadFrame.' + wc.data.values.templatePlatform + '&gameVariety=' + variety;
		}
		else {
...
wc.interface.replaceCurrentElement = function (nextElementType, nextChapterIndex) {
	...
	if (nextElementType == wc.interface.elementTypes.chapter) {
		...
		if (isDefined(mediaBulletins) && isDefined(discussionContent)) {
			if (wc.interface.doBullets && wc.interface.showBullets && wc.interface.elements.mediaBullets.hasBullets(nextChapter.pithyQuote)) {
		...
		if (typeof (wc.data.jsonData.chapters[nextChapterIndex].audio) != 'undefined' && wc.data.jsonData.chapters[nextChapterIndex].audio != null) {
			if (wc.interface.enableAudio && wc.interface.options.mediaPlayer.allowAudioAutoPlay && wc.data.jsonData.chapters[nextChapterIndex].chapterOptions.autoplay == 'true') {
	...
	// Mark S. change to make audio option persist

//alert("audio isactive " + wc.interface.elements.audioSpeechButton.isActive);

//alert("audio isdisabled " + wc.interface.elements.audioSpeechButton.isDisabled);

//alert("enableaudio " + wc.interface.enableAudio);

	if (wc.interface.enableAudio == false) {

//alert("activating audio button");
//			wc.interface.elements.audioSpeechButton.activate();
////		wc.interface.elements.audioSpeechButton.disable();
//alert("activated");
	...
	if (wc.data.jsonData.chapters[nextChapterIndex].chapterOptions.pagination == "false") {
		try {
			...
			document.getElementById('Button_Note').style.display = 'none';
			document.getElementById('Button_ResizeFont').style.display = 'none';
			...
			document.getElementById('Button_AudioSpeech').style.display = 'none';
			document.getElementById('Button_EnableBullets').style.display = 'none';
			document.getElementById('Button_DisableBullets').style.display = 'none';
			document.getElementById('Button_Policy').style.display = 'none';
			document.getElementById('Button_Help').style.display = 'none';
		}
		catch (err) { }
	}
	...
	wc.interface.elements.wideChapter.resizeFont(true);
	wc.interface.elements.chapter.resizeFont(true);
	wc.interface.currentElement.displayType = wc.interface.displayTypes.Container;
};
	...
	if (elementType == wc.interface.elementTypes.game) {
		...
		$('[data-droppable="true"]').droppable({
			...
			drop: function (event, element) {
				...
				if (matches == data.problems.length) {
					//console.log('DONE');
		...
		switch (data.variety) {
			case wc.interface.linkBulletinTypes.Code.varietyCode:
				linkBulletinTypeName = wc.interface.linkBulletinTypes.Code.varietyName;
				break;
			case wc.interface.linkBulletinTypes.News.varietyCode:
				linkBulletinTypeName = wc.interface.linkBulletinTypes.News.varietyName;
				break;
			case wc.interface.linkBulletinTypes.FAQs.varietyCode:
				linkBulletinTypeName = wc.interface.linkBulletinTypes.FAQs.varietyName;
				break;
			case wc.interface.linkBulletinTypes.Policy.varietyCode:
				linkBulletinTypeName = wc.interface.linkBulletinTypes.Policy.varietyName;
				break;
			case wc.interface.linkBulletinTypes.DosAndDonts.varietyCode:
				linkBulletinTypeName = wc.interface.linkBulletinTypes.DosAndDonts.varietyName;
				break;
			case wc.interface.linkBulletinTypes.CodeCompanion.varietyCode:
				linkBulletinTypeName = wc.interface.linkBulletinTypes.CodeCompanion.varietyName;
				break;
		}
		...
		templateHtml = templateHtml.replaceTag('Content', 'LinkBulletin_Url', data.url || '');
		templateHtml = templateHtml.replaceTag('Content', 'LinkBulletin_TypeName', linkBulletinTypeName || '');
		templateHtml = templateHtml.replaceTag('Content', 'Chapter_Footer', wc.data.jsonData.chapters[wc.interface.currentElement.chapterIndex].footer || '');


		//AdiA - hack for IpdaFrame for do's and dont's
		...
		currentElementopacityTween.start();
		wc.interface.elements.linkBulletin.resizeFont(true);
		wc.interface.currentElement.elementType = elementType;
	}
...
//alert("has video");
			...
			wc.mediaPlayer.video.videoParams.width = data.video.width;
			wc.mediaPlayer.video.videoParams.height = data.video.height;
//alert("video src " + data.video.src + " alttext " + data.video.alternateText);
//alert("video transcript " + data.video.transcript);
	...
	//wc.interface.scrollWhiteboard();
	//setTimeout(wc.interface.scrollWhiteboard, 1500);
	wc.interface.applyScroll('#LayerElementContainer', true);
...
wc.interface.navigateToElementAfterLastChapter = function () {
	...
	if (wc.interface.currentElement.elementType == wc.interface.elementTypes.chapter || wc.interface.currentElement.elementType == wc.interface.elementTypes.popQuiz) {
...
wc.interface.elements = {
	...
	bookmark:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'bookmark_Container');
		}
	},
	gamechoice:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'gameChoiceBody');
		}
	},
	acknowledgment:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'Acknowledgment_Body_Content,Acknowledgment_Refusal_Content');
		}
	},
	popquiz:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'PopQuiz_Introduction,PopQuiz_Video_Transcript,PopQuiz_Question_Container_Body_Questions,popQuiz_Body_Feedback');
		}
	},
	term:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			if (resizeToPreviousFrameForm)
				wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'termContents');
			else
				wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'termContents,Chapter_Discussion_MediaBulletins,Chapter_Discussion_Content');
		}
	},
	policy:
	{
		display: function () {
			...
			var template = wc.data.templateElements.policy;
			template = template.replaceTag('Content', 'Policy_Text', splittedDescription[splittedDescription.length - 1]);
			wc.interface.generalDisplayMethods.displaySingleLayer(wc.interface.elementTypes.policy, template, this.resizeFont);
		},
		close: function () {
			wc.interface.generalDisplayMethods.hideLayerAndShowCurrentElement();
		},
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'policyContents');
		}
	},
	help:
	{
		...
		if (helpStrings.length == 9) {
			...
			templateHtml = templateHtml.replaceTag('Content', 'Help_Notes', helpStrings[3]).replace(/\\"/g, '"');
			...
			wc.interface.generalDisplayMethods.displaySingleLayer(wc.interface.elementTypes.note, templateHtml, this.resizeFont);
		},
		close: function () {
			wc.interface.generalDisplayMethods.hideLayerAndShowCurrentElement();
		},
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'h1,h2,h3,h4,h5,h6,h7,h8,h9');
		}
	},
	tableOfContents:
	{
		...
		select: function (option) {
			...
			wc.interface.generalDisplayMethods.selectListItem("mediaBullet_", option);
		...
		display: function () {
			var mainTemplate = wc.data.templateElements.tableOfContents;
			...
			for (var i = 0; i < wc.data.jsonData.chapters.length; i++) {
				...
				var clickFunction = "";
				var itemColor = "";
				if (i <= wc.interface.currentElement.chapterIndex) {
			...
			mainTemplate = mainTemplate.replaceTag('Content', 'TableOfContents_Items', allItemsTemplate);
			wc.interface.generalDisplayMethods.displaySingleLayer(wc.interface.elementTypes.tableOfContents, mainTemplate, this.resizeFont);
		},
		close: function () {
			wc.interface.generalDisplayMethods.hideLayerAndShowCurrentElement();
		},
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'QuizQuestion_Choices_Content');
		}
	},
	notes:
	{
		display: function () {
			var template = wc.data.templateElements.note;
			if (isDefined(wc.interface.storage.loadfromStorage(wc.lms.objInfo.aicc_sid)))
				template = template.replaceTag('Content', 'NotesText', wc.interface.storage.loadfromStorage(wc.lms.objInfo.aicc_sid));
			else
				template = template.replaceTag('Content', 'NotesText', '');
			wc.interface.generalDisplayMethods.displaySingleLayer(wc.interface.elementTypes.note, template, this.resizeFont);
		},
		...
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'notesText,notesTextArea');
		}
	},
	fonts:
	{
		fontSizeIncrease: 3,
		currentFontSizeIncrease: 0,
		fontSizeSteps: 0,
		oldElement: '',
		resetToDefault: function () {
			this.fontSizeIncrease = 3;
			this.currentFontSizeIncrease = 0;
			this.fontSizeSteps = 0;
		},
		resizeFont: function (id) {
			this.fontSizeSteps++;
			if (this.fontSizeSteps > 3) {
				this.currentFontSizeIncrease = -9;
				this.fontSizeSteps = 0;
			}
			else {
				this.currentFontSizeIncrease = 3;
			}

			var ids = id.split(',');
			//console.log(ids);
			//console.log("this.currentFontSizeIncrease " + this.currentFontSizeIncrease);
			//console.log("this.fontSizeSteps " + this.fontSizeSteps);
			for (var i = 0; i < ids.length; i++) {
				if (isDefined(document.getElementById(ids[i]))) {
					var element = $('#' + ids[i]);
					this.resize(element);
				}
			}
		...
		resize: function (elm) {
			//console.log(elm);
			var font = elm.css('font-size');
			//console.log(this.currentFontSizeIncrease);
			//console.log(font);
			if (font.indexOf('em') != -1) {
				//console.log('return');
				return;
			}
			font = font.replace('px', '');
			elm.css('font-size', (parseInt(font) + this.currentFontSizeIncrease) + 'px');
			//console.log(elm.css('font-size'));
		},
		resizeFrameFont: function (resizeToPreviousFrameForm, elementsToResize) {
			if (resizeToPreviousFrameForm) {
				if (wc.interface.elements.fonts.fontSizeSteps > 0) {
					var stepsToResize = wc.interface.elements.fonts.fontSizeSteps;
					wc.interface.elements.fonts.resetToDefault();
					for (var step = 1; step <= stepsToResize; step++) {
						if (isDefined(elementsToResize))
							this.resizeFont(elementsToResize);
					}
				}
			}
			else {
	...
	quiz:
	{
		...
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'QuizQuestion_Body_Content,QuizQuestion_Body_Feedback,Quiz_Body_Content,quizLeftPage,Quiz_Body_Content,Quiz_Body_Feedback_Text');
		},
		executeSelectedOption: function () {
			if (this.selectedOption != -1) {
	...
	linkBulletin:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			if (resizeToPreviousFrameForm)
				wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'LinkBulletin_Body_Content');
			else
				wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'Chapter_Discussion_MediaBulletins, Chapter_Discussion_Content,LinkBulletin_Body_Content');
		}
	},
	chapter:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'Chapter_Discussion_MediaBulletins,Chapter_Discussion_Content');
		}
	},
	wideChapter:
	{
	...
	//Implemented by AdiA - IPadFrame
	memorandumChapter:
	{
		resizeFont: function (resizeToPreviousFrameForm) {
			wc.interface.elements.fonts.resizeFrameFont(resizeToPreviousFrameForm, 'memorandumDiscutionContainer');
		}
	},
	//Implemented by AdiA - IPadFrame
	fullTextButton:
	{
		isActive: false,
		enable: function () {
			var Button_FullText = document.getElementById('Button_DisableBullets');
			if (!isDefined(Button_FullText))
				return false;

			wc.interface.elements.general.setCss(Button_FullText, "fulltextButton");
			wc.interface.elements.general.setClickEvent(Button_FullText, "wc.interface.actionHandler.disableBullets();", wc.interface.actionHandler.disableBullets);
			//Button_FullText.setAttribute("class", "fulltextButton");
			//Button_FullText.setAttribute("onclick", "wc.interface.actionHandler.disableBullets();");
			Button_FullText.disabled = false;
			this.isActive = false;
		},
		disable: function () {
			var Button_FullText = document.getElementById('Button_DisableBullets');
			if (!isDefined(Button_FullText))
				return false;
			wc.interface.elements.general.setCss(Button_FullText, "fulltextButton_Disabled");
			wc.interface.elements.general.setClickEvent(Button_FullText, "return false;", wc.interface.elements.general.emptyFunction);
			//Button_FullText.setAttribute("class", "fulltextButton_Disabled");
			//Button_FullText.setAttribute("onclick", "return false;");
			Button_FullText.disabled = true;
			this.isActive = false;
		},
		activate: function () {
			var Button_FullText = document.getElementById('Button_DisableBullets');
			if (!isDefined(Button_FullText))
				return false;
			wc.interface.elements.general.setCss(Button_FullText, "fulltextButton_Active");
			wc.interface.elements.general.setClickEvent(Button_FullText, "wc.interface.actionHandler.disableBullets();", wc.interface.actionHandler.disableBullets);
			//Button_FullText.setAttribute("class", "fulltextButton_Active");
			//Button_FullText.setAttribute("onclick", "wc.interface.actionHandler.disableBullets();");
			Button_FullText.disabled = true;
			this.isActive = true;
		}
	...
	audioSpeechButton:
	{
		...
		isDisabled: false,
		enable: function () {
			var Button_AudioSpeech = document.getElementById('Button_AudioSpeech');
			if (!isDefined(Button_AudioSpeech))
				return false;

			wc.interface.elements.general.setCss(Button_AudioSpeech, "audioSpeech");
			wc.interface.elements.general.setClickEvent(Button_AudioSpeech, "wc.interface.elements.audioSpeechButton.toggleAudioSpeechOn();", wc.interface.elements.audioSpeechButton.toggleAudioSpeechOn);
			//Button_AudioSpeech.setAttribute("class", "audioSpeech");
			//Button_AudioSpeech.setAttribute("onclick", "wc.interface.elements.audioSpeechButton.toggleAudioSpeechOn();");
			Button_AudioSpeech.disabled = false;
			this.isActive = false;
		...
		disable: function () {
			...
			if (!isDefined(Button_AudioSpeech))
				return false;
			wc.interface.elements.general.setCss(Button_AudioSpeech, "audioSpeech_Disabled");
			wc.interface.elements.general.setClickEvent(Button_AudioSpeech, "return false;", wc.interface.elements.general.emptyFunction);
			//Button_AudioSpeech.setAttribute("class", "audioSpeech_Disabled");
			//Button_AudioSpeech.setAttribute("onclick", "return false;");
			Button_AudioSpeech.disabled = true;
			this.isActive = false;
		...
		activate: function () {
			...
			wc.interface.elements.general.setCss(Button_AudioSpeech, "audioSpeech_Active");
			wc.interface.elements.general.setClickEvent(Button_AudioSpeech, "wc.interface.elements.audioSpeechButton.toggleAudioSpeechOff();", wc.interface.elements.audioSpeechButton.toggleAudioSpeechOff);
			//Button_AudioSpeech.setAttribute("class", "audioSpeech_Active");
			//Button_AudioSpeech.setAttribute("onclick", "wc.interface.elements.audioSpeechButton.toggleAudioSpeechOff();");
			Button_AudioSpeech.disabled = false;
			this.isActive = true;
			this.isDisabled = false;
		},
		toggleAudioSpeechOff: function () {
			wc.interface.enableAudio = false;
			wc.interface.showBullets = false;
			wc.interface.actionHandler.pauseAudio();
			wc.interface.elements.audioSpeechButton.enable();
			wc.interface.elements.bulletsButton.enable();
			wc.interface.elements.fullTextButton.activate();
			wc.interface.elements.playButton.disable();
			wc.interface.elements.mediaBullets.toggleMainDiscussionOn();
		},
		toggleAudioSpeechOn: function () {
			wc.interface.enableAudio = true;
			wc.interface.elements.audioSpeechButton.activate();
			wc.interface.elements.playButton.enable();
			wc.interface.elements.pauseButton.disable(true);
		}
	...
	bulletsButton:
	{
		...
		enable: function () {
			var Button_Bullets = document.getElementById('Button_EnableBullets');
			if (!isDefined(Button_Bullets))
				return false;

			wc.interface.elements.general.setCss(Button_Bullets, "bulletsButton");
			wc.interface.elements.general.setClickEvent(Button_Bullets, "wc.interface.actionHandler.enableBullets();", wc.interface.actionHandler.enableBullets);
			//Button_Bullets.setAttribute("class", "bulletsButton");
			//Button_Bullets.setAttribute("onclick", "wc.interface.actionHandler.enableBullets();");
			Button_Bullets.disabled = false;
			this.isActive = false;
		},
		disable: function () {
			var Button_Bullets = document.getElementById('Button_EnableBullets');
			if (!isDefined(Button_Bullets))
				return false;
			wc.interface.elements.general.setCss(Button_Bullets, "bulletsButton_Disabled");
			wc.interface.elements.general.setClickEvent(Button_Bullets, "return false;", wc.interface.elements.general.emptyFunction);
			//Button_Bullets.setAttribute("onclick", "return false;");
			Button_Bullets.disabled = true;
			this.isActive = false;
		},
		activate: function () {
			var Button_Bullets = document.getElementById('Button_EnableBullets');
			...
			wc.interface.elements.general.setCss(Button_Bullets, "bulletsButton_Active");
			wc.interface.elements.general.setClickEvent(Button_Bullets, "wc.interface.actionHandler.enableBullets();", wc.interface.actionHandler.enableBullets);
			//Button_Bullets.setAttribute("class", "bulletsButton_Active");
			//Button_Bullets.setAttribute("onclick", "wc.interface.actionHandler.enableBullets();");
			Button_Bullets.disabled = false;
			this.isActive = true;
		}
	...
	playButton:
	{
		isVisible: true,
		enable: function () {
			var Button_Play = document.getElementById('Button_PlayAudio');
			if (!isDefined(Button_Play))
				return false;

			var hiddenDiv = document.getElementById('Button_PlayAudio_Div');
			if (isDefined(hiddenDiv)) {
				wc.interface.elements.general.setCss(Button_Play, "playButton");
				wc.interface.elements.general.setClickEvent(Button_Play, "wc.interface.actionHandler.playAudio(true);", wc.interface.actionHandler.playAudio, true);
				//Button_Play.setAttribute("class", "playButton");
				//Button_Play.setAttribute("onclick", "wc.interface.actionHandler.playAudio(true);");
			}

			Button_Play.disabled = false;
			Button_Play.style.display = '';
			this.isVisible = true;
		},
		disable: function (hide, forceHide) {
			var Button_Play = document.getElementById('Button_PlayAudio');
			if (!isDefined(Button_Play))
				return false;
			...
			var hiddenDiv = document.getElementById('Button_PlayAudio_Div');
			if (isDefined(hiddenDiv)) {
				wc.interface.elements.general.setCss(Button_Play, "playButton_Disabled");
				wc.interface.elements.general.setClickEvent(Button_Play, "return false;", wc.interface.elements.general.emptyFunction);
				//Button_Play.setAttribute("class", "playButton_Disabled");
				//Button_Play.setAttribute("onclick", "return false;");
				isOkToHide = false;

			}

			Button_Play.disabled = true;
			if ((hide && isOkToHide) || (forceHide == true)) {
	...
	pauseButton:
	{
		isVisible: true,
		enable: function () {
			var Button_Pause = document.getElementById('Button_PauseAudio');
			if (!isDefined(Button_Pause))
				return false;

			var hiddenDiv = document.getElementById('Button_PauseAudio_Div');
			if (isDefined(hiddenDiv)) {
				wc.interface.elements.general.setCss(Button_Pause, "pauseButton");
				wc.interface.elements.general.setClickEvent(Button_Pause, "wc.interface.actionHandler.pauseAudio(true);", wc.interface.actionHandler.pauseAudio, true);
				//Button_Pause.setAttribute("class", "pauseButton");
				//Button_Pause.setAttribute("onclick", "wc.interface.actionHandler.pauseAudio(true);");
			}

			Button_Pause.disabled = false;
			Button_Pause.style.display = '';
			this.isVisible = true;
		},
		disable: function (hide) {
			...
			var hiddenDiv = document.getElementById('Button_PauseAudio_Div');
			if (isDefined(hiddenDiv)) {
				wc.interface.elements.general.setCss(Button_Pause, "pauseButton_Disabled");
				wc.interface.elements.general.setClickEvent(Button_Pause, "return false;", wc.interface.elements.general.emptyFunction);
				//Button_Pause.setAttribute("class", "pauseButton_Disabled");
				//Button_Pause.setAttribute("onclick", "return false;");
			}
	...
	nextButton: {
		enable: function () {
			...
			var hiddenDiv = document.getElementById('Button_Next_Div');
			if (isDefined(hiddenDiv)) {
				Button_Next.setAttribute("class", "navigationImgButtonNext");
				Button_Next.setAttribute("onclick", "wc.interface.actionHandler.nextButton()");
			}
		...
		disable: function (hide) {
			...
			//AdiA -- HACK for iPadFrame
			var isOkToHide = true;
			var hiddenDiv = document.getElementById('Button_Previous_Div');
			if (isDefined(hiddenDiv)) {
				Button_Next.setAttribute("class", "navigationImgButtonNext_Disabled");
				wc.interface.elements.general.setClickEvent(Button_Next, "return false;", wc.interface.elements.general.emptyFunction);
				//Button_Next.setAttribute("onclick", "return false;");
				isOkToHide = false;
			}
	...
	previousButton: {
		enable: function () {
			...
			var hiddenDiv = document.getElementById('Button_Previous_Div');
			if (isDefined(hiddenDiv)) {
				wc.interface.elements.general.setCss(Button_Previous, "navigationImgButtonPrev");
				wc.interface.elements.general.setClickEvent(Button_Previous, "wc.interface.actionHandler.previousButton();", wc.interface.actionHandler.previousButton);
			}
		...
		disable: function (hide) {
			...
			var hiddenDiv = document.getElementById('Button_Previous_Div');
			if (isDefined(hiddenDiv)) {
				wc.interface.elements.general.setCss(Button_Previous, "navigationImgButtonPrev_Disabled");
				wc.interface.elements.general.setClickEvent(Button_Previous, "return false;", wc.interface.elements.general.emptyFunction);
				//Button_Previous.setAttribute("onclick", "return false;");
				isOkToHide = false;
			}
	...
	survey: {
		...
		setResponse: function (id, choices, choiceIndex, responseValue) {
			//choices are sent with pivotquestions, response value with multiple choice one answer

			//alert(id + "|" + choiceIndex + "|" + responseValue);
...
//  this is for pivot questions, so we return the right number for baxter, after we have gotten the proper text response
	...
	mediaBullets: {
		...
		parse: function (pithyQuote) {
			...
			//pithyQuote = pithyQuote.replace(/<a href="navigate:moveForward\(([0-9]+)\);">/gi, '<a href="javascript:void(0);" id="navigate_moveForward_$1" onclick="wc.interface.customNavigation.moveForward($1);">');
			//pithyQuote = pithyQuote.replace(/<a href="navigate:moveBackward\(([0-9]+)\);">/gi, '<a href="javascript:void(0);" id="navigate_moveBackward_$1" onclick="wc.interface.customNavigation.moveBackwards($1);">');
			//pithyQuote = pithyQuote.replace('<a href="navigate:exitChapters();">', '<a href="javascript:void(0);" id="navigate_exitChapters" onclick="wc.interface.navigateToElementAfterLastChapter();">');
		...
		parseBulletList: function (bulletListHtml, bulletinList, mediaBullets) {
					...
					if (bulletsMatches.length > 1) {
						...
						if (bulletsMatches.length > 1) {
							...
							if (bulletType == 'Bullet') {
								var bullet = {};
								var bulletAttributes = bulletsMatches[3];
								...
								bullet.content = bulletsMatches[4];
...
wc.interface.actionHandler = {
	...
	quizQuestion_MultipleChoiceOneAnswerSelected: function (choiceIndex, elementType) {
		...
		if (isDefined(Quiz_Body_Feedback_Container)) {
			...
			//var Quiz_Body_Feedback_ContainerBoundraries = wc.GetObjectBoundaries(Quiz_Body_Feedback_Container);
			...
			//	document.getElementById('Quiz_Element').style.height = Quiz_Body_Feedback_ContainerBoundraries.height;
			//}
		}
	...
	popQuiz_answerSelected: function (answerIndex, questionIndex) {
		...
		if (isDefined(feedbackContainerElement)) {
			...
			// Is this really needed? Don't understand the logic for this...
			// The popQuiz_Body_Feedback_Container is not a child of PopQuiz_Question_Container, so why do we need to increase the size of the PopQuiz_Question_Container?
			//			if (Quiz_Body_Feedback_ContainerBoundraries.height > Quiz_ElementBoundraries.height) {				
			//				document.getElementById('PopQuiz_Question_Container').style.height = (Quiz_Body_Feedback_ContainerBoundraries.height + 150) + 'px';
			//			}
	...
	enableBullets: function () {
		wc.interface.showBullets = true;
		wc.interface.elements.mediaBullets.toggleMainDiscussionOff();
		if (!wc.interface.elements.audioSpeechButton.isDisabled) {
			wc.interface.elements.audioSpeechButton.activate();
			if (wc.interface.elements.playButton.isVisible) {
				wc.interface.elements.playButton.enable();
			}
		}
		wc.interface.elements.bulletsButton.activate();
...
wc.interface.scrollWhiteboard = function () {
	//console.log('test');
	// check to see if there is any element that needs to support scrolling
	var scrollElement = document.getElementById('whiteboard'); // fixme: this should be done through a css class?
	...
	if (document.all) {
		...
		if (isDefined(scrollElement)) {
			...
			if (isDefined(closeButton)) {
				if (!document.all) {
					//document.body.addEventListener('touchmove', function (e) { e.preventDefault(); });
				}
				if (typeof iScroll != 'undefined') {
					var scroll = new iScroll(scrollElement);
				}
			}
			else {
				setTimeout(wc.interface.scrollWhiteboard, 500);
			}
		}
	}
};

wc.interface.memorandumscrollWhiteboard = function () {
	// check to see if there is any element that needs to support scrolling
	//var scrollElement = document.getElementById('whiteboardmemorandum'); // fixme: this should be done through a css class?
	...
	//	if (isDefined(scrollElement))
	//	{
	//		var memorandumDiscutionContainer = document.getElementById('memorandumDiscutionContainer');
	//		if (isDefined(memorandumDiscutionContainer))
	//		{
	//			if (typeof iScroll != 'undefined')
	//			{
	//				var scroll = new iScroll(scrollElement);
	//				wc.interface.elements.memorandumChapter.customizeScroll();
	//			}
	//		}
	//		else
	//		{
	//			setTimeout(wc.interface.memorandumscrollWhiteboard, 500);
	//		}
	//	}
	//}
};

wc.interface.linkBulettinScrollWhiteboard = function () {
	// check to see if there is any element that needs to support scrolling
	var scrollElement = document.getElementById('whiteboardLinkBulletin'); // fixme: this should be done through a css class?
	...
	if (document.all) {
		...
		if (isDefined(scrollElement)) {
			var container = document.getElementById('linkBulletinContent');
			if (isDefined(container)) {
				if (typeof iScroll != 'undefined') {
					var scroll = new iScroll(scrollElement);
					wc.interface.elements.linkBulletin.customizeScroll();
				}
			}
		}
		else {
			setTimeout(wc.interface.linkBulettinScrollWhiteboard, 500);
		}
	}
};
...
wc.interface.generalDisplayMethods = {
	displaySingleLayer: function (elementType, templateHtml, resizeFunction) {
		var layerElementContainer = document.getElementById('LayerElementContainer');
		var currentElementContainer = document.getElementById('CurrentElementContainer');
		if (document.all)	// IE
		{
			...
			layerElementContainer.style.left = wc.GetObjectBoundaries(currentElementContainer).right;
		}

		layerElementContainer.style.top = wc.GetObjectBoundaries(currentElementContainer).top;
		wc.interface.actionHandler.scrollToTop();

		if (wc.interface.layerData.elementType == -1 || (wc.interface.layerData.elementType != -1 && currentElementContainer.style.display == '')) {
			//Hide main layer
			wc.interface.layerData.initialize();
			currentElementopacityTween = new OpacityTween(currentElementContainer, Tween.regularEaseOut, 100, 0, 0.2);
			currentElementopacityTween.onMotionFinished = function () {
				currentElementContainer.style.display = 'none';
				layerElementContainer.style.display = '';
				layerElementContainer.innerHTML = templateHtml;
				if (isDefined(resizeFunction))
					resizeFunction(true);
				layerElementopacityTween = new OpacityTween(layerElementContainer, Tween.regularEaseOut, 0, 100, 0.2);
				layerElementopacityTween.start();
			};
			currentElementopacityTween.start();
			wc.interface.currentElement.elementType = elementType;
	...
	hideLayerAndShowCurrentElement: function () {
		var layerElementContainer = document.getElementById('LayerElementContainer');
		var currentElementContainer = document.getElementById('CurrentElementContainer');

		if (wc.interface.layerData.elementType == -1) {
			currentElementopacityTween = new OpacityTween(layerElementContainer, Tween.regularEaseOut, 100, 0, 0.2);
			currentElementopacityTween.onMotionFinished = function () {
				layerElementContainer.style.display = 'none';
				currentElementContainer.style.display = '';
			...
			currentElementopacityTween = new OpacityTween(layerElementContainer, Tween.regularEaseOut, 100, 0, 0.2);
			currentElementopacityTween.onMotionFinished = function () {
				wc.interface.displayLayer(wc.interface.layerData.elementType, wc.interface.layerData.data);
				layerElementopacityTween = new OpacityTween(currentElementContainer, Tween.regularEaseOut, 0, 100, 0.2);
				layerElementopacityTween.start();
			};
			currentElementopacityTween.start();
